; $Id: bpch2nc.pro,v 1.6 2008/03/24 14:51:16 bmy Exp $
;-----------------------------------------------------------------------
;+
; NAME:
;        BPCH2NC
...
; or phs@io.as.harvard.edu with subject "IDL routine bpch2nc"
;-----------------------------------------------------------------------
...
pro Bpch2Nc, InFile, OutFile, Verbose=Verbose, DiagN=DiagN, _EXTRA=e
   ...
   for T = 0L, N_Elements( Tau0 ) - 1L do begin
      ...
      if ( Is_3D ) then if ( GridInfo.LMX eq 1 ) then Is_3D = 0
      ...
      if ( Is_3D ) then begin
         ...
         ; Add one more than the max dimension of ZDIM to
         ; account for the sigma/eta edge array (bmy, 12/20/07)
      ...
      TitleStr = 'NetCDF file created by BPCH2NC (GAMAP v2-12+)'
      ...
      ; Write sigma/eta centers & edges to netCDF file (3-D grids)
      ; but only write up to the max # of vertical levels
      if ( Is_3D ) then begin
         ...
         Tmp_NC_ZdimE = B2N_GetNcDim( Tmp, ZDim, NC_ZDim )
         ...
         ; TMP_NC_ZDIMC returns the netCDF dimension 
         ; corresponding to # of sigma or eta centers
         Tmp_NC_ZdimC = B2N_GetNcDim( Tmp, ZDim, NC_ZDim )
         ...
         if ( ModelInfo.Hybrid ) then begin
            ...
            ; Define netCDF ETA edges variable
            ...
            ; Define netCDF sigma edges variable
      ...
      for D = 0L, N_Elements( ThisDataInfo ) - 1L do begin
         ...
         ; Check if this name is not already use one or more
         ; time. This is done to deal with rare cases of data blocks
         ; with the same description (category, tracername, timestamp)
         ; in bpch files (phs, 01/23/08) 
         if ( D ne 0L ) then begin
            ijk = 2
            while total(Name_save[0:D-1L] eq Name) gt 0. do begin
               Name = Name + strtrim(ijk, 2)
               ijk++ 
            endwhile
         endif
      ...
      if ( Is_3D ) then begin
         ...
         if ( IsHybrid ) then begin
            ...
            if ( Verbose ) then print, vEta, 'ETAE', Format=F2
            ...
            if ( Verbose ) then print, vSig, 'SIGE', Format=F2
