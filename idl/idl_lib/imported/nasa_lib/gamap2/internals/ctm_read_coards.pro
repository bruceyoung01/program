; $Id: ctm_read_coards.pro,v 1.1.1.1 2007/07/17 20:41:48 bmy Exp $
;-----------------------------------------------------------------------
;+
; NAME:
;        CTM_READ_COARDS
...
pro CRC_Read_Global_Atts, fId, Info, DimLon,     DimLat,    DimLev, $
   ...
   ;
   ; NOTES:
   ; Bug fix: Lev eq -1 should be Lev[0] eq -1 (bmy, 3/6/06)
   ;====================================================================
...
pro CRC_Save_Data, InType,       InGrid,      Time,      Delta_Time,  $
   ...
   ; If DATA lacks a "time" dimension, then add one for use below
   if ( N_Elements( SData ) eq 2 ) then begin
      Data  = Reform( Data, SData[0], SData[1], 1 )
      SData = Size( Data, /Dim )
   endif else if ( N_Elements( SData ) eq 3 ) then begin
