; $Id: ctm_read_data.pro,v 1.1.1.1 2007/07/17 20:41:47 bmy Exp $
;-----------------------------------------------------------------------
;+
; NAME:
;        CTM_READ_DATA
...
; or phs@io.as.harvard.edu with subject "IDL routine ctm_read_data"
;-----------------------------------------------------------------------
...
pro ctm_read_data,data,param1,filepos,xsize,ysize,zsize,  $
    ...
    ; Quick fix: assuming 4D will always be read
    ; with the DATAINFO structure (phs, 6/21/07)
    tsize = 0
    ...
    if(n_params() eq 2) then begin
        ...
        ysize = param1.dim[1]
        zsize = param1.dim[2]
        tsize = param1.dim[3]  ; For 4-D data blocks (phs, 6/21/07)
        scale = param1.scale
    ...
    ; make sure data has at least one spatial dimension of ge 1
...
entry_ok:
    ...
    ; Size the data block for 2D, 3D or 4D
    if ( tsize gt 1 ) then                           $ 
       data = FltArr( xsize, ysize, zsize, tsize )   $
    else if ( zsize gt 1 ) then                      $
       data = FltArr( xsize, ysize, zsize )          $
    else                                             $
       data = FltArr( xsize, ysize )
