; $Id: ctm_read3db_header.pro,v 1.1.1.1 2007/07/17 20:41:47 bmy Exp $
;-----------------------------------------------------------------------
;+
; NAME:
;        CTM_READ3DB_HEADER
...
; with subject "IDL routine ctm_read3db_header"
;-----------------------------------------------------------------------
...
; @gamap_cmn.pro
  ...
  ;pprint=1
    ...
    ; For binary punch file w/ 4-D data blocks (phs, 6/21/07)
    Add1D = Long( FileType eq 106 )
    ...
    while (newpos lt fsize-1) do begin
       ...
       ;------------------------------------------------------------
       ; Prior to 6/21/07:
       ; Modified for 4-D data blocks (phs, 6/21/07)
       ;dim = lonarr(6)
       ;------------------------------------------------------------
       dim = lonarr( 6 + Add1D )
       skip = 0L
...
;### skip = 4L * ( dim[0]*dim[1]*dim[2] ) + 8L    ( 8 is for EOL in fortran )
       ...
       ;-------------------------------------------------------------
       struc(count).dim      = Add1D ? fix( dim[0:3] ) : [ fix(dim[0:2]), 0 ]
       struc(count).first    = fix( Add1D ? dim[4:6] : dim[3:5] ) 
       ...
       case ( FileType ) of 
          ...
          106  : Struc(Count).Format = 'BINARY PUNCH v2.0: 4-D data blocks'
       ...
       if (keyword_set(PPRINT)) then begin
          ...
          ;-----------------------------------------------------------
          print,'     dimensions : ',Add1D ? dim[0:3] : dim[0:2]
          print,'  first indices : ',Add1D ? dim[4:6] : dim[3:5]
          print,'           skip : ',skip
          print,'         newpos : ',newpos
       endif
