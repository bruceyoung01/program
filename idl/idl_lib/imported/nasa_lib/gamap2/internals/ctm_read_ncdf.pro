; $Id: ctm_read_ncdf.pro,v 1.5 2008/05/30 14:02:03 bmy Exp $
;-----------------------------------------------------------------------
;+
; NAME:
;        CTM_READ_NCDF
...
; phs@io.as.harvard.edu with subject "IDL routine ctm_read_ncdf"
;-----------------------------------------------------------------------
...
pro CRN_Get_Tracer, VarName, Category, TrcName, Tracer
   ...
   ;### Kludge for benchmarks (bmy, 5/23/08)
   ;if ( VarName eq 'PS-PTOP__PSURF' ) then VarName = 'PEDGE-S__PEDGE'
   ;if ( VarName eq 'PEDGE-S__PSURF' ) then VarName = 'PEDGE-S__PEDGE'

   ; Look for "__" separating CATEGORY from TRACERNAME
   ...
   if ( Ind[0] ge 0 ) then begin
      ...
      ; Look for "::" separating CATEGORY from TRACERNAME
      ; (as is the case with older files created under IDL 5.x)
      Ind = StrPos( VarName, '::' )
...
pro CRN_Read_Global_Atts, fId, ModelInfo, GridInfo, Tau0, Tau1, Is_GMI
   ...
   ; Determine if this file was created by BPCH2GMI
   Is_GMI    = ( StrPos( Title, 'BPCH2GMI' ) ge 0 )
   ...
   ; If the "NLayers" attribute is 1, then don't add vertical
   ; information to the GRIDINFO structure (bmy, 12/19/07)
   NoVert    = ( NLayers eq 1 )

   ; Create ModelInfo and GridInfo structures
   ModelInfo = CTM_Type( Model, Res=[DI,DJ], NLayers=NLayers )
   GridInfo  = CTM_Grid( ModelInfo, No_Vertical=NoVert )
...
pro CRN_Save_Data, InType,   InGrid, Tau0,         Tau1,         $
   ...
   ;
   ; NOTES:
   ; (1) Assumes global-size data blocks (which is what routine BPCH2NC 
   ;      and BPCH2GMI generates).
   ; (2) Shift data created by BPCH2GMI by 180 degrees so that the 1st
   ;      longitude will occur on the date line.  This will make it
   ;      consistent with GAMAP usage. (bmy, 8/21/07)
   ;====================================================================
   ...
   ; Test if this data file was created by BPCH2GMI
   if ( Is_GMI ) then begin

      ;---------------------------------------------------
      ; Files created with BPCH2GMI have the 1st 
      ; longitude starting at 0 degrees (prime meridian), 
      ; so we must shift the data by 180 degrees
      ;---------------------------------------------------
      
      ; Shift data by 180 degrees
      case ( N_Elements( SData ) ) of 
         2: Data = Shift( Data, SData[0]/2L, 0L     )
         3: Data = Shift( Data, SData[0]/2L, 0L, 0L )
      endcase

      ; Format string for CTM_MAKE_DATAINFO
      Format = 'BPCH2GMI netCDF'

   endif else begin

      ;---------------------------------------------------
      ; Files created with BPCH2NC have the 1st 
      ; longitude starting at 180 degrees (date line),
      ; so we don't have to shift the data
      ;---------------------------------------------------
...
function CTM_Read_NCDF, Ilun, FileName, FileInfo, DataInfo, _EXTRA=e
   ...
   CRN_Read_Global_Atts, fId, InType, InGrid, Tau0, Tau1, Is_GMI
   ...
   for vId = 0L, VarInfo.NVars-1L do begin
      ...
      ; Skip over index arrays: LON, LAT, SIG[CE], ETA[CE]
      if ( Name eq 'LON'  ) then goto, Next
      if ( Name eq 'LAT'  ) then goto, Next
      if ( Name eq 'ETAC' ) then goto, Next
      if ( Name eq 'ETAE' ) then goto, Next
      if ( Name eq 'SIGC' ) then goto, Next
      if ( Name eq 'SIGE' ) then goto, Next
