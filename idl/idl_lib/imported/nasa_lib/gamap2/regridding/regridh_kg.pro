; $Id: regridh_kg.pro,v 1.1.1.1 2007/07/17 20:41:34 bmy Exp $
;-----------------------------------------------------------------------
;+
; NAME:
;        REGRIDH_KG
...
; or phs@io.as.harvard.edu with subject "IDL routine regridh_kg"
;-----------------------------------------------------------------------


pro RegridH_Kg, InFileName=InFileName,               $
   ...
   for D = 0L, N_Elements( DataInfo )-1L do begin
      ...
      if ( Keyword_Set( Use_Saved_Weights ) ) $
         then US = Use_Saved_Weights          $
         else US = 1L - FirstTime

      ; Regrid the data
      OutData = CTM_RegridH( InData, InGrid, OutGrid, /Double, Use_Saved=US )
