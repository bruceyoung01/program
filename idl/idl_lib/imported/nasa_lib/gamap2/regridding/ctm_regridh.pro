; $Id: ctm_regridh.pro,v 1.1.1.1 2007/07/17 20:41:33 bmy Exp $
;-----------------------------------------------------------------------
;+
; NAME:
;        CTM_REGRIDH
...
function CTM_RegridH, Data, OldGrid, NewGrid,                               $
   ...
   for L = 0L, LMX - 1L do begin
      ...
      if ( FromFineToCoarse ) then begin
         ...
         ; Multiply by areas ratio (if necessary) so that the quantity
         ; being regridded will be area-dependent (phs, 10/24/06)
         ; - part 1 -
...
NextII0:
         ...
         ; being regridded will be area-dependent (phs, 10/24/06)
         ; - part 2 -
         if ( Per_Unit_Area ) then $
            NewData[*,*,L] = NewData[*,*,L] / NewArea
      ...
      ;=================================================================
         ...
         ; Multiply by areas ratio (if necessary) so that the quantity 
         ; being regridded will NOT be area-dependent (phs, 10/24/06)
         ; -  part 1 -
         if ( not Per_Unit_Area ) then TmpData = TmpData / OldArea
         ...
         for I = 0L, CoarseGrid.IMX - 1L do begin
            ...
            for II = 0L, N_X - 1L do begin
               ...
               FracData = TmpData[I,J] * Weight[I,J,II,JJ]
