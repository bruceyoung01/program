; $Id: regridh_aircraft_nox.pro,v 1.1.1.1 2007/07/17 20:41:32 bmy Exp $
;-----------------------------------------------------------------------
;+
; NAME:
...
function RA_Read_AirNox, GridInfo, FileName

   ;====================================================================
   ; Internal function RA_READ_AIRNOX reads the aircraft NOx data on
...
pro RA_Write_AirNox, Data, GridInfo, FileName

   ;====================================================================
   ; Internal function RA_WRITE_AIRNOX writes the regridded
   ; aircraft NOx data to disk. (bmy, 4/10/03)
   ;====================================================================
   ...
   for L = 1L, 26L          do begin
   for J = 1L, GridInfo.JMX do begin
   for I = 1L, GridInfo.IMX do begin
            
      ; Only write out nonzero points
      if ( Data[I-1,J-1,L-1] gt 0D ) then begin
         PrintF, Ilun, I, J, L, Data[I-1,J-1,L-1] / 6d0, $
            Format='(3i3,e10.3)'
      endif
   endfor
   endfor
...
pro RegridH_Aircraft_NOx, InFileName=InFileName,       $
   ...
   InData = RA_Read_AirNox( InGrid, InFileName )
   ...
   ; Truncate and write to output file (ASCII)
   RA_Write_AirNox, OutData, OutGrid, OutFileName
