; $Id: velocity_field.pro,v 1.2 2008/03/24 14:51:20 bmy Exp $
;-----------------------------------------------------------------------
;+             
; NAME:              
;        VELOCITY_FIELD          
...
; or phs@io.harvard.edu with subject "IDL routine velocity_field"
;-----------------------------------------------------------------------    
...
pro Velocity_Field, U, V, X, Y,                                       $
                    ...
                    DateLine=DateLine,     PlotPosition=PlotPosition, $
                    _EXTRA=e
   ...
   if ( N_Elements( PlotPosition ) eq 0 ) then PlotPosition = [0., 0., 1., 1.]

   Dots     = Keyword_Set( Dots )
   DateLine = Keyword_Set( DateLine )
   ...
   for I = 0L, N_Elements( Good ) - 1L do begin 
      ...
      ;### Quick kludge, skip bad points
      if ( DateLine AND ( X1 lt X[0] ) ) then goto, Next

      ; now check if point are in the plot window (phs,3/20/08)
      tail = Convert_Coord( X0, Y0, /To_NOrm, /data )
      head = Convert_Coord( X1, Y1, /To_NOrm, /data )

      if ( Tail[0] lt PlotPosition[0] ) then goto, Next
      if ( Tail[0] gt PlotPosition[2] ) then goto, Next
      if ( Tail[1] lt PlotPosition[1] ) then goto, Next
      if ( Tail[1] gt PlotPosition[3] ) then goto, Next
                                       
      if ( Head[0] lt PlotPosition[0] ) then goto, Next
      if ( Head[0] gt PlotPosition[2] ) then goto, Next
      if ( Head[1] lt PlotPosition[1] ) then goto, Next
      if ( Head[1] gt PlotPosition[3] ) then goto, Next
