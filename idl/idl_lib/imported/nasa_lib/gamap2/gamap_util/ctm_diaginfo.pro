; $Id: ctm_diaginfo.pro,v 1.2 2008/07/17 14:08:52 bmy Exp $
;-----------------------------------------------------------------------
;+
; NAME:
;        CTM_DIAGINFO
...
; or phs@io.harvard.edu with subject "IDL routine ctm_diaginfo"
;-----------------------------------------------------------------------
...
pro CTM_DiagInfo, DiagN, DiagStru,                                  $    
   ...
   if ( Keyword_Set( Force_Reading ) OR  $
      ...
      ; Use the FILE_WHICH routine of IDL to first look for "diaginfo.dat"
      ; in the current directory.  If not found there, then FILE_WHICH will
      ; look in the directories specified in the !PATH variable. (bmy, 7/11/07)
      IF n_elements(Filename) eq 0 then $
         FileName = File_Which( 'diaginfo.dat', /Include_Current_Dir )
