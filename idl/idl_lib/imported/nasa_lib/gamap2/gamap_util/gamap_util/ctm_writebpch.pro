; $Id: ctm_writebpch.pro,v 1.1.1.1 2007/07/17 20:41:24 bmy Exp $
;-----------------------------------------------------------------------
;+
; NAME:
;        CTM_WRITEBPCH
...
; or phs@io.harvard.edu with subject "IDL routine ctm_writebpch"
;-----------------------------------------------------------------------
...
pro CTM_WriteBpch, DataInfo, FileInfo,                $
   ...
   if ( N_Elements( FileInfo ) eq 0 ) then begin
      ...
      Four     =  ( FileInfo[0].FileType eq 106 )
   endelse
   ...
   if ( not( Keyword_set( Append ) ) ) then begin
      FTI      = Str2Byte( Four ? 'CTM bin 4D' : 'CTM bin 02', 40 )
   ...
   for I = 0L, N_Elements( DataInfo ) - 1L do begin
      ...
      ; Settings from DATAINFO for 4D or 3D data blocks (phs, 6/21/07)
      ; NOTE: FIRST is in FORTRAN notation (array indices start from 1)
      if ( Four ) then begin
         Dimensions = Long( DataInfo[I].Dim[0:3] )
         First      = Long( DataInfo[I].First )    
         Dim        = [ Dimensions, First ]
         Skip       = 4L *  ( Dim[0] * Dim[1] * Dim[2] * Dim[3] ) + 8L
      endif else begin
