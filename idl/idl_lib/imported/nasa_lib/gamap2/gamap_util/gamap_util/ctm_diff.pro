; $Id: ctm_diff.pro,v 1.1.1.1 2007/07/17 20:41:25 bmy Exp $
;-----------------------------------------------------------------------
;+
; NAME:
;        CTM_DIFF
...
; or phs@io.harvard.edu with subject "IDL routine ctm_diff"
;-----------------------------------------------------------------------
...
pro CTM_Diff, DiagN, $
   ...
   CTM_Get_Data, DataInfo1, DiagN, $
      Tracer=Tracer_1, Tau0=Tau0_1, Ilun=Ilun_1, FileName=File_1, _EXTRA=e
   ...
   CTM_Get_Data, DataInfo2, DiagN, $
      Tracer=Tracer_2, Tau0=Tau0_2, Ilun=Ilun_2, FileName=File_2, _EXTRA=e
...
;   endelse
   ...
   if ( NeedsRegridding ) then begin
      if ( YesNo( 'Dimensions do not agree!  Regrid DATA2?' ) ) then begin
         Data2 = CTM_RegridH( TmpData2, Grid2, Grid1, _EXTRA=e )
      endif else return
   ...
   ; Compute the min & max difference values for return (bmy, 5/1/07)
   Range = [ Min( Diff, Max=M ),  M ]
