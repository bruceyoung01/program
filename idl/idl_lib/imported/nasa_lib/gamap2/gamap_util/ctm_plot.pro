; $Id: ctm_plot.pro,v 1.4 2008/04/03 20:12:16 bmy Exp $
;--------------------------------------------------------------------
;+
; NAME:
;        CTM_PLOT
...
; or phs@io.harvard.edu with subject "IDL routine ctm_plot"
;--------------------------------------------------------------------
...
pro CTM_Plot, DiagN,                                                $
   ...
   PixelPlot = Keyword_Set( Sample   )   ; phs (4/2/08)
   ...
   ;         ###### WARNING: it is CENTERS and not EDGES (phs, 4/2/08)
   ...
   ;===================================================================
   ; Call CTM_TRACERINFO to get various tracer quantities
   ...
   CTM_TracerInfo, ThisDataInfo.Tracer, $
      Name=Name, MolC=CNum, MolWt=MolWt, Is_Edged=Is_Edged, _EXTRA=e
   ...
   case ( PlotType ) of
     ...
     'XZ' : begin
        ...
        ; need to use bottom and top edges instead of centers for
        ; pixel plots (phs, 4/2/08)
        YRange = [ min(Ymid), max(Ymid) ]
        if PixelPlot then $
           Yrange = gridinfo.zedge[[UP[0], UP[n_elements(UP)-1]+1 ]]
        
        ; LON are grid centers. So we need to add half grid for pixel
        ; plots (phs, 4/2/08)
        if (n_elements(LON) eq 2) then begin
           XRange = Lon
           if PixelPlot then xrange += [-1., 1] * 0.5 * Gridinfo.DI 
        endif
     ...
     'YZ' : begin
        ...
        ; LAT are grid centers. So we need to add half grid for pixel
        ; plots (phs, 4/2/08)
        if (n_elements(Lat) eq 2) then begin
           XRange = Lat
           if PixelPlot then xrange += [-1., 1] * 0.5 * Gridinfo.DJ 
        endif
   ...
   if ( not ChkStru(GridInfo,'LMX') ) then begin
      ...
      ;-----------------------------------------------------------------------
      ; Prior to 5/17/07:
      ; We now need to account for those fields that are defined on
      ; the level edges instead of on the level centers (bmy, 5/17/07)
      ...
      ;-----------------------------------------------------------------------
      if ( Is_Edged ) then begin

         ; For fields that are defined on the level edges then
         ; we need also to devine ALT & PRS on level edges
         MaxLev = MaxLev < ( GridInfo.LMX ) 
         Alt    = [ GridInfo.Zedge( MinLev-1 ), GridInfo.ZEdge( MaxLev ) ]
         Prs    = [ GridInfo.PEdge( MinLev-1 ), GridInfo.PEdge( MaxLev ) ]

      endif else begin 

         ; For fields that are defined on the level centers
         ; then define ALT & PRS with ZMID & PMID
         MaxLev = MaxLev < ( GridInfo.LMX - 1 )
         Alt    = [ GridInfo.ZMid( MinLev-1 ), GridInfo.ZMid( MaxLev - 1 ) ]
         Prs    = [ GridInfo.PMid( MinLev-1 ), GridInfo.PMid( MaxLev - 1 ) ]
...
; print,'PlotPosition=',PlotPosition
   ...
   if ( SData eq 2 ) then begin
      ...
      if ( PlotType eq 'XY' ) then begin
         ...
         ; Comment that line. That was not used and now prevents to pass dlon
         ; from the calling routine like GAMAP (phs, 3/19/08)
;         DLon = GridInfo.DI
      endif
