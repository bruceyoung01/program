; $Id: gamap.pro,v 1.4 2008/07/17 14:14:15 bmy Exp $
;-----------------------------------------------------------------------
;+
; NAME:
;        GAMAP
...
; or phs@io.harvard.edu with subject "IDL routine gamap"
;-----------------------------------------------------------------------
...
pro GAMAP, DiagN,                                                   $
   ...
   if (FirstTime) then begin
      print,'============================================================='
      print
      print,'   WELCOME  TO  G A M A P  -----  Version 2.12, Jul 2008'
      print
   ...
   ; - if FILENAME exists, then strip white space
   if (keyword_set(NoFile) AND ptr_valid(pGlobalFileInfo) ) $
      then undefine,FileName $
      else if (n_elements(FileName) eq 0)        $
         then FileName = ''                      $
         else FileName = StrTrim( FileName, 2 )
   ...
   ;=================================================================
   ; Check if any file carries 4D data blocks and warn that 
   ; first 3D block will be used (phs, 11/20/07)
   ;=================================================================
   FileInfo = *( PGlobalFileInfo )
   LUNs     = DataInfo.Ilun 

   ; Any 4D file?
   FourDInd = Where( FileInfo.Filetype eq 106, count)

   ; Any of them correspond to DataInfo?
   if ( Count ge 1 ) then begin

      ; CMSET_OP is like a "where" command for arrays
      IndF = CmSet_Op( [FileInfo[FourDInd].ILun], 'AND', [LUNs],  $
                       Count=Count, /Index )

      ; Print warning mesage
      for klm = 1L, count do begin
         message,'WARNING: Data in file ' + $
                  FileInfo[ FourDInd[ IndF[klm-1] ] ].FileName + $
                 ' are 4D... Will use only 3D: [*,*,*,0] !', $
                 /Continue
      endfor
   endif
