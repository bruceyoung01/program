; $Id: ctm_boxsize.pro,v 1.1.1.1 2007/07/17 20:41:28 bmy Exp $
;-----------------------------------------------------------------------
;+
; NAME:
...
; or phs@io.harvard.edu with subject "IDL routine ctm_boxsize"
;-----------------------------------------------------------------------
...
function ctm_boxsize, GridInfo,  Radius,                                  $
   ...
   ;### Debug test: more robust formulation (bmy, 11/13/06)
   ;if ( IJ ) then begin
   ;   Area  = FltArr( GridInfo.IMX, GridInfo.JMX )
   ;   F_IMX = Double( GridInfo.IMX )
   ;
   ;   ; Use more robust sine formula
   ;   for J = 0L, GridInfo.JMX-1L do begin
   ;      Area[*,J] = ( 2d0* PI * double( Radius )^2 / F_IMX ) * $
   ;         ( sin( GridInfo.YEdge(J+1) * D2R ) -             $
   ;           sin( GridInfo.Yedge(J  ) * D2R ) )
   ;   endfor
   ;endif
