...
;  explicitly, then the AUTODERIVATIVE=0 keyword must be passed to
;  MPFIT, and then the model function must be declared with an
;  additional positional parameter as described below.  As a practical
;  matter, it is often sufficient and even faster to allow MPFIT to
;  calculate the derivatives numerically, and so AUTODERIVATIVE=0 is
;  not necessary.
;
...
;  implementation quirk, MPFIT expects the negative of that value.  To
;  repeat, MPFIT expects the sign of DP should have the same sign as
;  the gradient of F, and the opposite sign of the gradient of RESID.
;
...
;  number of parameters.  -DP[i,j] is the derivative at the ith
...
;   Add COMPATIBILITY section and add credits to copyright, CM, 13 Dec 2007
;
;  $Id: mpfit.pro,v 1.50 2007/12/15 14:37:25 craigm Exp $
;-
; Original MINPACK by More' Garbow and Hillstrom, translated with permission
; Modifications and enhancements are:
; Copyright (C) 1997-2003, 2004, 2005, 2006, 2007, Craig Markwardt
...
;  prof_start = systime(1)
  ...
  if maxiter EQ 0 then goto, TERMINATE
