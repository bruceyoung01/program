  ...
  ; set up color levels
	...
	if ( count gt 0 ) then tmptau(result) = 8
     ...
     plot, [xl, xr], [ybb, ytt], /nodata, xrange=[xl, xr], $
             ...
             ytitle='Latitude (deg) ', $
             position= [xa, ya, xb, yb], color=1, xthick=3,$
             ythick=3, charsize=1.0, charthick=3, xstyle=1, ystyle=1,$
             xticks=8, xminor=5
      
      xyouts, (xa+xb)/2, yb+0.03, titlename, /normal, color=1,$
              align=0.5 
      
      map_set, 0, xcenter,  /continent, $
       /grid, charsize=1, mlinethick = 4, color=1,con_color=1,$
...
; 
       oplot, [xl, xr], [ybb, ybb], thick=5, color=1
       oplot, [xl, xr], [ytt, ytt], thick=5, color=1
       oplot, [xl, xl], [ytt, ytt], thick=5, color=1
       oplot, [xr, xr], [ybb, ybb], thick=5, color=1
...
;	 color=16, charthick=5
	...
	if barticks(n_levels-i) ge 1.0 then begin 
	 ...
	 format='(f3.1)'), /REMOVE_ALL),  color=1,/normal, charsize=1.0,  charthick=3,$
	 align=0.5
	endif

	if barticks(n_levels-i) lt 1.0 then begin 
	 xyouts, xb+i*dx+ddx+dx, yb+dy+ddy,  '!6'+ string(barticks(n_levels-i),$
	 format='(f4.2)'),  color=1,/normal, charsize=1.0,  charthick=3,$
	 align=0.5
	endif
...
;	 color=16, charthick=5
    ...
    if (strmatch (titlename, '*Radius*129*' )) then begin 
       ...
       for i = 0, 162 do begin
        ...
        scolor = 15
        plots, location(1, i), location(0, i), psym=8, color=1, /data 
       endfor 
     ...
     ;  xyouts,  -91, 20, '2.54 cm', color=scolor, /data, align=0.5
       xyouts, xb-3*dx, yb+dy+ddy, '!4l!6m', charsize=1.0,  $
            charthick=3, align=0.5, color = 1, /normal   
