; $ID: process_day_ivaot_npp.pro V01 04/16/2012 10:01 BRUCE EXP$
;
;******************************************************************************
; Purpose: read the list of NPP IVAOT granuel file names, and
;          find out how many days are these granules 
;          collected. the start name index and ending index
;          for each day. 
;
; FILENAME :
; IVAOT_npp_d20120401_t1622012_e1623254_b02217_c20120401184825574745_noaa_ops.h5
...
; NOTES:
; =============================================================================
; (1 ) ORIGINALLY WRITTEN BY SOMEONE. (NOT CLEAR)
; (2 ) MODIFIED BY BRUCE. (04/16/2012)
;******************************************************************************

PRO process_day_ivaot_npp, infile, Nday, AllFileName, StartInx, EndInx, $
...
;  READ YEAR, MONTH, AND DAY FROM FILE NAME
         OneF = AllFileName(i)                       ; current day
         YearC = fix(strmid (OneF, 11, 4))
         MonthC= fix(strmid (OneF, 15, 2))
         DayC  = fix(strmid (OneF, 17, 2))
;  CONVERT YEAR, MONTH, DAY TO JULIAN DAY
         JulianD = YMD2DN(YearC, MonthC, DayC)
         NxF    = AllFileName(i+1)                      ; next day
         YearC1 = fix(strmid (NxF, 11, 4))
         MonthC1= fix(strmid (NxF, 15, 2))
         DayC1  = fix(strmid (NxF, 17, 2))
         IF (YearC1  NE 0 AND $
             MonthC1 NE 0 AND $
             DayC1   NE 0) THEN BEGIN
         NxJD   = YMD2DN(YearC1, MonthC1, DayC1)
         ENDIF ELSE BEGIN
         NxJD   = 0
         ENDELSE
      ...
      ; get the month correct
       ...
       ; judget if on the same day
          if ( JulianD ne NxJD  ) then begin
             ...
             MON(DI) = fix(Month)
             Date(DI) = fix(Day) 
             TimeS(DI) = strmid(AllFileName(SSINX), 21, 7) 
             TimeE(DI) = strmid(AllFileName(I), 30, 7)
             DayNum(DI) = tmpday
             DAYNAME(DI) = Month + Day + $
                           'JD' + string(JulianD, format = '(I3)')
             SSINX = i+1
             DI = DI + 1
          endif
    endfor
