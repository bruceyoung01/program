...
;  file_calc = '~rjp/Asim/run_v7-02-01_NA_nested_1x1/IJ-AVG_2001_01-12.1x1.bpch'
...
;     Yosemite (CA), Hoover (CA)
;     data[133,*] = -999.
;     data[48,*]  = -999.

     conc    = (improve_calc.ecpi_conc+improve_calc.ecpo_conc)*12.
...
;     data[48,*]  = -999.
     ...
     conc    = (improve_calc.ocpi_conc+improve_calc.ocpo_conc)*12. $
             + (improve_calc.soa1_conc)*120.11             $
             + (improve_calc.soa2_conc)*120.11             $
             + (improve_calc.soa3_conc)*180.17

;    omc
;     conc    = (castnet_calc.ocpi_conc+castnet_calc.ocpo_conc)*12.*1.4 $
;             + (castnet_calc.soa1_conc)*150.                   $
;             + (castnet_calc.soa2_conc)*160.                   $
;             + (castnet_calc.soa3_conc)*220.
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  !Y.THICK=4
   ...
   if (!D.NAME eq 'PS') then $
       Open_device, file='carb_us_'+DXDY+'_scatter.ps', /PS, /Portrait, $
  ...
  xxp = 0.70
  xyouts, xxp, 0.86, 'Annual', /normal, color=1, charsize=charsize,$
          charthick=charthick, alignment=0.5
  xyouts, xxp, 0.68, 'DJF', /normal, color=1, charsize=charsize,$
          charthick=charthick, alignment=1
  xyouts, xxp, 0.50, 'MAM', /normal, color=1, charsize=charsize,$
          charthick=charthick, alignment=1
  xyouts, xxp, 0.33, 'JJA', /normal, color=1, charsize=charsize,$
          charthick=charthick, alignment=1
  xyouts, xxp, 0.15, 'SON', /normal, color=1, charsize=charsize,$
          charthick=charthick, alignment=1

  xyouts, 0.22, 0.97, 'EC', /normal, color=1, charsize=charsize,$
          charthick=charthick, alignment=0.5
  xyouts, 0.50, 0.97, 'OC', /normal, color=1, charsize=charsize,$
          charthick=charthick, alignment=0.5
;  xyouts, 0.79, 0.97, 'NH!d4!u+!n', /normal, color=1, charsize=charsize,$
;          charthick=charthick, alignment=0.5

  xyouts, 0.4, 0.01, 'OBSERVATION [!4l!3g m!u-3!n]', /normal, color=1, $
          charsize=charsize, charthick=charthick, alignment=0.5

  xyouts, 0.04, 0.43, 'MODEL [!4l!3g m!u-3!n]', $
