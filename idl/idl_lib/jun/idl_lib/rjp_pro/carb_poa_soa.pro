  ...
  case spec of
    ...
    'POA' : begin
            ...
            range=[0.,6.]
               ...
               omc  = (calc.ocpi_conc_globe+calc.ocpo_conc_globe)*12.*1.4 $
                    + (calc.soa1_conc_globe)*150.                   $
                    + (calc.soa2_conc_globe)*160.                   $
                    + (calc.soa3_conc_globe)*220.
...
;============================================================================
 ...
 pro makeplot, spec, improve_obs, castnet_obs, castnet_calc, modelinfo, $
  ...
  if spec eq 'SO4' or spec eq 'OMC' or spec eq 'EC' or spec eq 'DUST' or $
     spec eq 'NO3' or spec eq 'POA' or spec eq 'SOA' then begin
...
;  CBPosition = [0.2,0.36,0.7,0.38]
  ...
  xyouts, 0.5*(p[0]+p[2]), 0.96, Name, alignment=0.5, color=1, $
...
; Observations are in ug/m3
  ...
  if (!D.Name eq 'PS') then $
      Open_device, file='carb_2dplot.ps', /ps, /color, /landscape, $
       encapsulated=0

   omargin = [ 0.1,  0.15, 0.17, 0.05 ]

   !p.multi[4] = 1  ; column major
   multipanel, row=2, col=3, omargin=omargin, Margin=[0.01,0.01,0.01,0.01]


  makeplot, 'OMC', improve_obs, castnet_obs, castnet_calc, modelinfo, $
    NoGXLabels=[1,0], NoGYLabels=[0,0], /cbar
  makeplot, 'POA', improve_obs, castnet_obs, castnet_calc, modelinfo, $
    NoGXLabels=[1,0], NoGYLabels=[1,1], unit='!C[!4l!3g m!u-3!n]', /cbar
  makeplot, 'SOA', improve_obs, castnet_obs, castnet_calc, modelinfo, $
    NoGXLabels=[1,0], NoGYLabels=[1,1], unit='!C[!4l!3g m!u-3!n]', /cbar
