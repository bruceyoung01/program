...
; Observations are in ug/m3
  ...
  file_calc = '~rjp/Asim/run_v7-02-01_NA_nested_1x1/STD_2001_01-12.1x1.bpch'
  ...
  if N_elements(imp_so4) eq 0 then begin
     data    = castnet_obs.so4
     conc    = castnet_calc.so4_conc*96.
  ...
  if N_elements(imp_no3) eq 0 then begin
     data    = castnet_obs.no3
     conc    = castnet_calc.nit_conc*62.
     time    = castnet_calc.time
     lat     = castnet_obs.lat  
     lon     = castnet_obs.lon
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  !Y.THICK=4
   ...
   if (!D.NAME eq 'PS') then $
       Open_device, file='ioa_castnet_'+DXDY+'_scatter.ps', /PS, /Portrait, $
