...
;     If N_elements(YYMM) eq 0 then return, 0
   ...
   ; Retriev met fields first
      CTM_Get_Data, AirdInfo, 'BXHGHT-$', Filename=file_aird, tracer=2004 ; #/m3
  ...
  if n_elements(nlayer) eq 0 then nlayer=1
    ...
    case spec of
      'OMC' : conc = (data.ocpi_conc + data.ocpo_conc)*12.*1.4 + $
                     ...
                     (data.soa2_conc)*160. + $
                     (data.soa3_conc)*220. 
      'SO4' : conc = (data.so4_conc)*96.
      'NIT' : conc = (data.nit_conc)*62.
      'TC'  : conc = (data.bcpi_conc + data.bcpo_conc)*12.     + $
                     (data.ocpi_conc + data.ocpo_conc)*12.*1.4 + $
                     (data.soa1_conc)*150. + $
                     (data.soa2_conc)*160. + $
                     (data.soa3_conc)*220. 
      else  : conc = data.conc
      ...
      omc[i0[0]:i0[0]+i1[0],i0[1]:i0[1]+i1[1]]  = conc
...
;      title = '!4D!3'+spec + ' concentrations in '+zzz+' mb '+title
      title = '!4D!3'+spec + ' in surface air for Jul-Aug, 2004'
...
;=============================================================================
 ...
 pro grim, spec, confiles0, confiles1, Nlayer, limit=limit, $
     mindata=mindata, maxdata=maxdata, unit=unit, nogylabel=nogylabel, $
     ndiv=ndiv

    ZZ      = strtrim(nlayer,2)
    For d = 0, n_elements(confiles0)-1L do begin
      ...
      if D eq 0 then diff = omc0-omc1 else diff = diff + (omc0-omc1)

    Endfor

    diff = diff / float(n_elements(confiles0))
;    diff = region_only(diff, region='UScont')

    multipanel, position=pos
    dx = (pos[2]-pos[0])*0.2
    dy = (pos[3]-pos[1])*0.1
    cbposition = [0.2,pos[1]-dy*2,0.8,pos[1]-dy]
    cbposition = [0.2,0.15,0.8,0.24]

    margin = [0.02,0.,0.02,0.]
    plot_region, diff, limit=limit, title=title, /cbar, $
         divis=ndiv, mindata=mindata, maxdata=maxdata, unit=unit , $
         cbformat='(F4.1)', /sample, min_valid=1.E-5, $
         nogylabel=nogylabel, margin=margin, cbposition=cbposition
...
;=============================================================================
    ...
    m_s_dir = '/users/ctm/rjp/Asim/icartt/Run_v7-04-03/output/out_trop/'
    confiles0 = 'X'
    confiles0 = [confiles0, collect(m_s_dir+'ctm.bpch.200407*')]
    confiles0 = [confiles0, collect(m_s_dir+'ctm.bpch.200408*')]
    confiles0 = confiles0[1:*]

    m_s_dir = '/users/ctm/rjp/Asim/icartt/Run_v7-04-03_nofire/output/out_wet/'
    confiles1 = 'X'
    confiles1 = [confiles1, collect(m_s_dir+'ctm.bpch.200407*')]
    confiles1 = [confiles1, collect(m_s_dir+'ctm.bpch.200408*')]
    confiles1 = confiles1[1:*]

  if !D.name eq 'PS' then $
    open_device, file='del_aero_wet.ps', /color, /ps, /landscape


    limit = [24., -150., 70., -62.]
    multipanel, row=1, col=2, omargin=[0.04,0.25,0.01,0.25]
    Nlayer  = 1

    spec    = 'TC'  &   unit = '!4l!3g m!u-3!n'
    mindata = 0
    maxdata = 3.
    ndiv    = 7
    grim, spec, confiles0, confiles1, Nlayer, limit=limit, $
        mindata=mindata, maxdata=maxdata, unit=unit, ndiv=ndiv

    spec    = 'SO4'  &  unit = '!4l!3g m!u-3!n'
    mindata = 0
    maxdata = 1.
    ndiv    = 6
    grim, spec, confiles0, confiles1, Nlayer, limit=limit, $
        mindata=mindata, maxdata=maxdata, unit=unit, /nogylabel, ndiv=ndiv
