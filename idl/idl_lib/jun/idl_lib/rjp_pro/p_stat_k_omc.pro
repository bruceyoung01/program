...
;======================================================================
 ...
 ; Plotting begins
  ...
  row = 1
  col = 2
  multipanel, row=row, col=col

  Pos = cposition(col,row,xoffset=[0.1,0.1],yoffset=[0.3,0.3], $
        xgap=0.02,ygap=0.15,order=0)

  cfac = 1.2
  if !D.name eq 'PS' then $
    open_device, file='stat_k_omc.ps', /color, /ps, /landscape

    fld = imp.r_knon_carb[2]  ; R between k and omc
    mindata = 0.
    maxdata = 1.

    mapplot, fld, imp, pos=pos[*,0], mindata=mindata, maxdata=maxdata, $
    cbformat='(F3.1)', unit=' ', cfac=cfac
    ...
    sid  = where(rarr ge Rcri)

    info = imp[sid]
    fld  = info.r_knon_carb[0]  ; slope between k and omc
    mindata = 0.
    maxdata = 100.

    mapplot, fld, info, pos=pos[*,1], mindata=mindata, maxdata=maxdata, $
    cbformat='(F4.0)', unit=' ', cfac=cfac, /nogylabel

  xyouts, 0.43, 0.34, '(a)', color=1, /normal, charsize=charsize, charthick=charthick
  xyouts, 0.84, 0.34, '(b)', color=1, /normal, charsize=charsize, charthick=charthick
