; $Id: ctm_retrieve_data.pro,v 1.1.1.1 2007/07/17 20:41:47 bmy Exp $
;-----------------------------------------------------------------------
;+
; NAME:
;        CTM_RETRIEVE_DATA
...
; or phs@io.as.harvard.edu with subject "IDL routine ctm_retrieve data"
;-----------------------------------------------------------------------
...
Insert_Data:
    ...
    ; just for safety, test if newdata has more than 1 element(
    ;--------------------------------------------------------------------
    ; Prior to 7/20/04:
    ; Now account for data blocks that are a single point (bmy, 7/20/04)
    ;if (n_elements(newdata) lt 2) then  $
    ;--------------------------------------------------------------------
    if (n_elements(newdata) lt 1) then  $
