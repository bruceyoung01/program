; $Id: ctm_read_eosgr.pro,v 1.2 2007/12/10 20:15:58 bmy Exp $
;-----------------------------------------------------------------------
;+
; NAME:
;        CTM_READ_EOSGR
...
; or phs@io.as.harvard.edu with subject "IDL routine ctm_read_eosgr"
;-----------------------------------------------------------------------
...
pro CRE_Get_DimInfo, gId,      Info, FileName, ModelInfo, $
   ...
   if ( StrPos( FileName, 'c402_rp'     ) ge 0 ) then ModelName = 'GEOS4'
   if ( StrPos( FileName, 'c403_cer'    ) ge 0 ) then ModelName = 'GEOS4'
   if ( StrPos( FileName, 'GEOS4'       ) ge 0 ) then ModelName = 'GEOS4'
   if ( StrPos( FileName, 'DAS.llk.asm' ) ge 0 ) then ModelName = 'GEOS4'
   ...
   ; Convert date & time to TAU values, depending on file type
   if ( StrPos( FileName, 'DAS.llk.asm' ) ge 0 ) then begin

      ;--------------------------------------------------------
      ; The DAS.llk.asm file time is referenced to 1/1/1993
      ;--------------------------------------------------------

      ; Compute Astronomical Julian Date from the TIME array
      Time = ( Time / 86400d0 ) + JulDay( 1, 1, 1993 ) - 0.5d0

      ; Convert to Y/M/D/h/m/s
      CalDat, Time, M, D, Y, H, Mi, S      

      ; Construct NYMD and NHMS 
      NYMD = Y*10000L + M *100L + D
      NHMS = H*10000L + Mi*100L ;+ S

      ; Convert to TAU values
      Tau0 = DblArr( TMX )
      for T = 0L, TMX-1L do begin
         Tau0[T] = NYMD2Tau( NYMD[T], NHMS[T], /Geos )
      endfor

   endif else begin

      ;--------------------------------------------------------
      ; For other files, just get the date from the filename
      ;--------------------------------------------------------
