; $Id: yboxplot.pro,v 1.1.1.1 2007/07/17 20:41:50 bmy Exp $
;-----------------------------------------------------------------------
;+
; NAME:
;        YBOXPLOT
...
; or phs@io.harvard.edu with subject "IDL routine yboxplot"
;-----------------------------------------------------------------------


pro yboxplot,data,group=group,mingroup=mingroup,maxgroup=maxgroup,  $
...
; ### CHANGES 
    xrange = [min(tmpdat),max(tmpdat)]
    ytickval = findgen(ngroups)
 ;  ytickval = [-0.5,ytickval,ngroups-0.5]
    ytickval = [ytickval]
    nticks = n_elements(ytickval)-1
    
    ; print,'Diagnostics: ngroups=',ngroups,' nticks=',nticks,$
    ;     'ytickval=',ytickval,' uselabel=',uselabel
...
; re-set tick control to default
   ...
   ytickval=0
    ...
    if (not keyword_set(overplot)) then begin
       plot,[0],[0],/nodata,ystyle=5,xrange=xrange,   $
           color=color,yrange=yrange,_EXTRA=e

    ; draw and annotate y-axis
       axis,!x.window(0),yaxis=1,/norm,color=color, $
             yticks=nticks,ytickv=ytickval, $
             ytickname=replicate(' ',nticks+1), $
             yminor=-1,_EXTRA=e

   ;   plots,[!x.crange(1),!x.crange(1)],yrange,color=color,thick=!x.thick
       plots,[!x.window(1),!x.window(1)],!y.window,/norm, $
             color=color,thick=!y.thick
   
       dummy = fltarr(n_elements(ytickval))+1.e-10 
       normval = convert_coord(dummy,ytickval,/data,/to_normal)
       xval = fltarr(n_elements(normval))+!x.window(0)

       if(n_elements(orientation) eq 0) then orientation = 0
       align = 1.0
       yshift = -0.01
       xshift = -0.02
       if (orientation ne 0) then begin
          align=0.5
          yshift=0.0
          xshift = -0.02
       endif
       ...
       xyouts,xval+xshift,normval(1,*)+yshift, $
...
; overlay boxes and whiskers
    ...
    yth = (!y.crange(1)-!y.crange(0))*0.005
    mth = (!x.crange(1)-!x.crange(0))*0.005
    ; determine y position of N marks
    xlp = !x.crange(1)-0.07*(!x.crange(1)-!x.crange(0))
    ; take care of logarithmic plots
    if (!x.type eq 1) then xlp = exp(xlp * alog(10.D))
    ...
    ; 25%-75%
        ...
        if (fillcolor(0) ge 0) then $
           polyfill,[p(1),p(3),p(3),p(1),p(1)], $
                 [left,left,right,right,left],  $
                 color=fillcolor(gg)

        if (boxcolor(0) ge 0) then $
        plots,[p(1),p(3),p(3),p(1),p(1)], $
              [left,left,right,right,left],  $
              color=boxcolor

    ; overlay median
        plots,[p(2),p(2)],[left,right], $
              color=mediancolor,thick=medianthick

    ; overlay mean as symbol
        if (n_elements(meansymbol) gt 0) then $
             oplot, [mv], [center], psym=meansymbol(0), color=meancolor

    ; lower and upper extremes
    ; cut values if they exceed plot boundaries by more than 5 %
        lowbound = !x.crange(0)-0.05*(!x.crange(1)-!x.crange(0))
        upbound  = !x.crange(1)+0.05*(!x.crange(1)-!x.crange(0))
    ; take care of logarithmic plots
        ...
        if(p(0) lt p(1)) then $
        plots, [p(0),p(1)],[center,center],  $
               color=limitcolor,thick=!p.thick
        if(p(4) gt p(3)) then $
        plots, [p(3),p(4)],[center,center],  $
               color=limitcolor,thick=!p.thick
       ...
       if(keyword_set(printn)) then $
          xyouts,xlp,center,strtrim(c,2),align=0.5, $
