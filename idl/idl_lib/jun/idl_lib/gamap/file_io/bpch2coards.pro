; $Id: bpch2coards.pro,v 1.3 2007/12/10 20:12:46 bmy Exp $
;-----------------------------------------------------------------------
;+
; NAME:
;        BPCH2COARDS
...
pro Bpch2COARDS, InFile, OutFile, Verbose=Verbose, DiagN=DiagN, _EXTRA=e
   ...
   for T = 0L, D_Dim-1L do begin
      ...
      ; Save to netCDF file 
      NCDF_VarPut, fId, vTim, Tau0Today
      ...
      for N = 0L, N_Elements( vID_Uniq )-1L do begin
         ...
         for D = 0L, N_pId-1L do begin
            ...
            ;-----------------------------------------------------------------
            ; Prior to 7/13/06:
            ; Don't free the pointer to avoid problems later on (bmy, 7/13/06)
            ;Ptr_Free, Pointer
            ;-----------------------------------------------------------------
