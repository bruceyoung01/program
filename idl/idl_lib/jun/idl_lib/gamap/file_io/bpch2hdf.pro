; $Id: bpch2hdf.pro,v 1.1.1.1 2007/07/17 20:41:39 bmy Exp $
;-----------------------------------------------------------------------
;+
; NAME:
;        BPCH2HDF
...
; or phs@io.as.harvard.edu with subject "IDL routine bpch2hdf"
;-----------------------------------------------------------------------


pro Bpch2Hdf, InFile, OutFile, DiagN=DiagN, _EXTRA=e
   ...
   for T = 0L, N_Elements( Tau0 ) - 1L do begin
      ...
      ; Get vertical coordinates if necessary
      if ( Is_3D ) then begin
      ...
      ; Write vertical coordinates to disk (for 3-D data blocks)
      if ( Is_3D ) then begin
         if ( IsHybrid ) then begin
            HDF_SetSd, fId, EtaC, 'ETAC', Unit='Eta',  LongName='Eta Centers'
            HDF_SetSd, fId, EtaE, 'ETAE', Unit='Eta',  LongName='Eta Edges'
         endif else begin
      ...
      for D = 0L, N_Elements( ThisDataInfo ) - 1L do begin
         ...
         if ( not Ptr_Valid( Pointer ) ) then begin
            S = 'Invalid pointer for ' + $
                StrTrim( ThisDataInfo[D].Category, 2 ) + ' and ' + $
         ...
         ;--------------------------------------------
         ; Prior to 5/19/06
         ; Do not free pointer memory (bmy, 5/19/06)
