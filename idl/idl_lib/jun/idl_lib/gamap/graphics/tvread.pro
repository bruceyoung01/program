; $Id: tvread.pro,v 1.1.1.1 2007/07/17 20:41:35 bmy Exp $
;-----------------------------------------------------------------------
;+
; NAME:
;       TVREAD
...
FUNCTION TVREAD_ERROR_MESSAGE, theMessage, Traceback=traceback, $
   ...
   widgetsSupported = ((!D.Flags AND 65536L) NE 0) OR $
                      Float(!Version.Release) GE 5.3
   IF widgetsSupported THEN BEGIN
      ...
      ELSE BEGIN
         IF StrUpCase(callingRoutine) EQ "$MAIN$" $
            THEN answer = Dialog_Message(theMessage, _Extra=extra) $
            ELSE answer = Dialog_Message(StrUpCase(callingRoutine) + ": " $
                                         + theMessage, _Extra=extra)
      ENDELSE
   ENDIF ELSE BEGIN
...
FUNCTION TVREAD, xstart, ystart, ncols, nrows, $
   ...
   IF (thisVersion LT 5.3) OR (thisVersion GE 6.1) $
      THEN haveGif = 1 ELSE haveGIF = 0
   ...
   ;----------------------------------------------------------
   ;%%% Modification by Bob Yantosca
   ;%%% Remove NODIALOG keyword (bmy, 5/21/07)
   ...
   ;%%% Modification by Bob Yantosca
   ;%%% Only pop up a dialog box if FILENAME isn't passed (bmy, 5/21/07)
   if ( WriteImage AND N_Elements( FileName ) eq 0 ) $
      then Dialog = 1                                $
      else Dialog = 0
