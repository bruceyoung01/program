; $Id: ctm_get_data.pro,v 1.3 2007/11/20 20:35:15 bmy Exp $
;-------------------------------------------------------------
;+
; NAME:
;        CTM_GET_DATA
...
; or phs@io.harvard.edu with subject "IDL routine ctm_get_data"
;-------------------------------------------------------------
...
pro ctm_get_data,datainfo,diagn,  $
              ...
              internal_use=internal_use, Quiet=Quiet, _extra=e
...
@gamap_cmn.pro
    ...
    ; This will allow the value of FILENAME as selected by the dialog
    ; box to be passed back to the calling routine (phs, bmy, 11/20/07)
    ...
    if (test[0] ge 0) then begin
       print, 'diagstru... = ', diagstru.Category  
    ...
    if (force_reading) then begin
       ...
       ; Check if FILENAME contains wild cards (bmy, 2/6/06)
       Is_Wild = ( StrPos( FileName, '*' ) ge 0 ) $
                 ( StrPos( FileName, '?' ) ge 0 ) 

       ; If FILENAME is undefined, or contains wild cards, 
       ; then pop up a dialog box and have the user pick the 
       ; file name. (bmy, 2/6/06)
       if ( Is_Wild OR FileName eq '' ) then begin
       
          ; If there are wild cards, then copy FILENAME to 
          ; the filter mask for DIALOG_PICKFILE (bmy, 2/6/06)
          ...
          ; Dialog box popup: get user input (bmy, 2/6/06)
          FileName = Dialog_PickFile( File=FName,            $
                                      Filter=FMask,          $
                                      Title='Choose a file' )
          
          ; If user selected "CANCEL" button, then return 
          ; to the calling program (bmy, 2/6/06)
          if ( FileName eq '' ) then return
       endif

       ; Open file and get file type
       ctm_open_file,filename,use_fileinfo,use_datainfo,  $
                     cancelled=cancelled, _extra=e ; phs
