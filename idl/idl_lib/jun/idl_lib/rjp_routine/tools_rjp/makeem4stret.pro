...
pro Ebuilder, month, lon2b, lat2b, lon2, lat2, inter=inter, $
...
;...
 ...
 areaout = stc_sfcarea(lat2b,lon2b)
...
;openw,jjj,'../output/256x129/ems_stret.256x129.b8501.e8512.dat_f77b',/f77,/swap_endian,/get
 openw,jjj,'../output/256x129/ems_stret.256x129.b8501.e8512.dat_grads',/get
 openw,ilun,'../output/256x129/emission_f77b.txt', /get

 lon = fltarr(ilmm)
 lat = fltarr(ijmm)

 openr,il,'/data/eos4/stone/SGCTM/GRID/stretgrid_256x129.lon.lat_f77b',/f77,/swap_endian,/get
 readu,il,lon,lat
 free_lun,il

; setup grid for output (stretched grid)
;...Y direction
...
;...Note the center of leftmost model box is always 180 W...
 ...
 lon2 = fltarr(ilmm,ijmm)
 lat2 = fltarr(ilmm,ijmm)

 for j = 0, ijmm-1 do lon2(*,j) = lon  ; Center
 for i = 0, ilmm-1 do lat2(i,*) = lat 

 lon2b = fltarr(ilmm+1,ijmm+1)
 lat2b = lon2b


;..Calculate boundary grid for stretched grid
 latb = fltarr(ijmm)
 lonb = fltarr(ilmm)

 dx = lon(1:ilmm-1)-lon(0:ilmm-2)
 dy = lat(1:ijmm-1)-lat(0:ijmm-2)

 xp = where(dx eq min(dx))
 yp = where(dy eq min(dy(1:ijmm-3)))

 xp0 = min(xp)
 yp0 = min(yp)

  lonb(xp0) = dx(xp0)*0.5+lon(xp0)

  for i = xp0+1, ilmm-1 do begin
   ddx  = lon(i)-lonb(i-1)
   lonb(i) = lon(i)+ddx
  endfor

  for i = xp0-1, 0, -1 do begin
   lonb(i) = 2*lon(i+1)-lonb(i+1)
  endfor

  latb(yp0) = dy(yp0)*0.5+lat(yp0)

  for j = yp0+1, ijmm-1 do begin
    ddy = lat(j)-latb(j-1)
    latb(j) = lat(j)+ddy
    if (j eq ijmm-1) then latb(j) = 90.
  endfor

  for j = yp0-1, 0, -1 do begin
    latb(j) = 2*lat(j+1)-latb(j+1)
  endfor

 latb = [-90.,latb]
 lonb = [lonb(ilmm-1)-360.,lonb]

 for j = 0, ijmm do lon2b(*,j) = lonb  ; Boundary
 for i = 0, ilmm do lat2b(i,*) = latb 

 for i = 1, 12 do begin

 Ebuilder, i, lon2b, lat2b, lon2, lat2, inter=inter, $
    NOx=NOx,CO=CO,ISOP=ISOP,C2H6=C2H6,C3H8=C3H8,C2H4=C2H4,C3H6=C3H6,CH3COCH3=CH3COCH3
