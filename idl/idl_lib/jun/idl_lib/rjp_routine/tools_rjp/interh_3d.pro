function interh_3d, fdin, lon2, lat2, $
...
Case ndim of 
   ...
   3  :   km = dim(2)
       ...
       print, 'Input dimension is not correct', ndim
	 return,0
	 end
endcase
...
fdout = fltarr(im1,jm1,km)
 ...
 for i = 0, im1-1 do begin
  ...
  fdout(i,j,k)   = fdinst(ip  ,jp,   k)*(1.-xcof)*(1.-ycof) $
                 + fdinst(ip+1,jp,   k)*  xcof   *(1.-ycof) $
                 + fdinst(ip+1,jp+1, k)*  xcof   *   ycof   $
                 + fdinst(ip  ,jp+1, k)*(1.-xcof)*   ycof
 end
 end
 end
