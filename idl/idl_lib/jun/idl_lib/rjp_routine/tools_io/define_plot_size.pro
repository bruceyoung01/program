...
;-
     ...
     dthick    = 3.
     charthick = 1.5   ; character thickness
     dcharthick= 1
     charsize  = 1.5  ; character size
    scharsize  = 1.5
 ...
 if (!D.NAME eq 'PS') then scharsize = 1.2 ; small charater size
 if (!D.NAME eq 'PS') then charsize  = 1.4  ; character size
 if (!D.NAME eq 'PS') then Tcharsize = 1.8  ; Title character size
 if (!D.NAME eq 'PS') then symsize   = 1.   ; symbol size
 if (!D.NAME eq 'PS') then symthick  = 3    ; symbol thickness
