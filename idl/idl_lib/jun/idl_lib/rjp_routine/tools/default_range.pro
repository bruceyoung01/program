; $Id: default_range.pro,v 1.1.1.1 2003/10/22 18:09:40 bmy Exp $
;-------------------------------------------------------------
;+
; NAME:
...
function defrange_str2num,sarg,default
    ...
    ;------------------------------------------------------------------
    ; Prior to 7/13/01:
    ; Use updated STRREPL function from mgs (bmy, 7/13/01)
    ...
    ;------------------------------------------------------------------
    ; Prior to 7/13/01:
    ; Eliminate reference to obsolete STR_SEP function (bmy, 7/16/01)
    ;result = Float(Str_Sep( StrCompress( StrTrim( sarg, 2 ) ), ' ' ) )
    ;------------------------------------------------------------------
    ; Prior to 1/17/02:
    ;Result = StrSplit( StrCompress( StrTrim( SArg, 2 ) ), ' ', /Extract )
    ;------------------------------------------------------------------
