; $Id: tvread.pro,v 1.1.1.1 2003/10/22 18:09:37 bmy Exp $
;----------------------------------------------------------------------------
;+
; NAME:
;       TVREAD
...
;          quantization will use a cubic method of quantization. Applies 
;          to BMP, GIF, PICT, and PNG formats written from 24-bit displays.
;          (See the COLOR_QUAN documentation for details.)
;
;       /DITHER -> If this keyword is set the quantized image will be dithered.
;          Applies to BMP, GIF, PICT, and PNG formats written from
...
;           set to "idl". The file extension will be set automatically
;           to match the type of file created.
;
...
;           JPEG file.
;
;       /NODIALOG -> Set this keyword if you wish to avoid the 
;           DIALOG_PICKFILE dialog that asks you to name the output
;           file. This keyword should be set, for example, if you are 
...
;           WRITE_*** routines are also accepted via keyword inheritance.
;
; COMMON BLOCKS:
;
;       None
...
;-
;       bmy, 27 Nov 2002: GAMAP VERSION 1.52
;                         - Removed /NODIALOG keyword
;                         - GIF is supported for IDL 5.3 now
;                         - Only pop a dialog box if FILENAME isn't
;                           passed and we are writing a file
;                         - updated comments, cosmetic changes
;       bmy, 21 Mar 2006: GAMAP VERSION 2.05
;                         - IDL version 6.2 now allows GIF
;
;###########################################################################
;
; LICENSE
...
FUNCTION TVREAD_ERROR_MESSAGE, theMessage, Traceback=traceback, $
   ...
   IF Float(!Version.Release) GE 5.2 $
      THEN $
   ...
   IF widgetsSupported THEN BEGIN
      IF Keyword_Set(noName) $
         THEN answer = Dialog_Message(theMessage, _Extra=extra) ELSE BEGIN
         IF StrUpCase(callingRoutine) EQ "$MAIN$" $
            THEN answer = Dialog_Message(theMessage, _Extra=extra) $
         ELSE $
...
FUNCTION TVREAD, xstart, ystart, ncols, nrows, $
                 ...
                 JPEG=jpeg, $
                 ;-------------------------------------------
                 ;### Modification by Bob Yantosca
                 ;### Only pop a dialog box if FILENAME
                 ;### isn't passed (bmy, 12/2/02)
                 ;NoDialog=nodialog, $
                 ;-------------------------------------------
                 Order=order, $
                 PICT=pict, $
   ...
   ;-------------------------------------------------------------
   ;### Modification by Bob Yantosca (12/2/02)
   ;### IF thisVersion LT 5.3 THEN haveGif = 1 ELSE haveGIF = 0
   ;-------------------------------------------------------------
   ;### Modification by Bob Yantosca (3/21/06)
   ;### Version 5.3 has GIF support, 5.4 does not (bmy, 12/2/02)
   ;IF thisVersion LT 5.4 THEN haveGif = 1 ELSE haveGIF = 0
   ;-------------------------------------------------------------
   ;### Modification by Bob Yantosca (3/21/06)
   ; GIF is supported prior to IDL 5.4 was restored in IDL 6.2  
   if ( thisVersion lt 5.4 OR thisVersion ge 6.2 ) $
      then haveGif = 1                             $
      else haveGif = 0
   ...
   ;--------------------------------------------
   ;### Modification by Bob Yantosca
   ;### Remove NODIALOG keyword (bmy, 12/2/02)
   ...
   ;### Modification by Bob Yantosca 
   ;### Only pop a dialog box if FILENAME isn't passed (bmy, 12/2/02)
   if ( WriteImage AND N_Elements( FileName ) ne 1 ) $
      then Dialog = 1                                $
      else Dialog = 0
   ...
   ; Get the screen dump. 2D image on 8-bit displays. 
   ; 3D image on 24-bit displays.
