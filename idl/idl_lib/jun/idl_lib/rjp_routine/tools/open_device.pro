; $Id: open_device.pro,v 1.1.1.1 2003/10/22 18:09:40 bmy Exp $
;-------------------------------------------------------------
;+
; NAME:
;        OPEN_DEVICE
...
;
;        OPEN_DEVICE, /Z
;            Opens the IDL Z-buffer device.  The current color table 
;            will be preserved in the Z-buffer device.
;
...
pro Open_Device, Old_Device,                               $
   ...
   ;--------------------------------------------------------------------
   ; Prior to 8/27/01:
   ; Now use different settings for landscape or portrait (bmy, 8/27/01)
   ;if ( N_Elements( XSize    ) eq 0 ) then XSize    = 8.0
   ;if ( N_Elements( YSize    ) eq 0 ) then YSize    = 10.0
   ;if ( N_Elements( XOffSet  ) eq 0 ) then XOffSet  = 0.25
   ;if ( N_Elements( YOffSet  ) eq 0 ) then YOffSet  = 0.25
   ;--------------------------------------------------------------------
   ...
   if ( Keyword_Set( PS ) ) then begin
      ...
      if ( LandScape ) then begin 
         ...
         ;--------------------------------------------------------------
         ; Prior to 8/28/01
         ; This is the original call -- now pass XOFFSET, YOFFSET,
         ; XSIZE, YSIZE to the DEVICE call for /LANDSCAPE (bmy, 8/28/01)
