;-------------------------------------------------------------
; $Id: struinfo.pro,v 1.1.1.1 2003/10/22 18:09:39 bmy Exp $
;+
; NAME:
;        STRUINFO (function)
;
; PURPOSE:
;        Return information about structures. This routine is
;        designed to help handling variable structures of mixed
;        type.
;
; CATEGORY:
;        Structure Tools
;
; CALLING SEQUENCE:
;        info = STRUINFO(stru,/Flag [,RefIndex=RefIndex])
;
; INPUTS:
;        STRU -> a structure  
;
; KEYWORD PARAMETERS:
;        /FLAG: One of the follwoign keywords:
;        - NAMES -> return variable names as spelled in structure tags
;        - ORIGINAL_NAMES -> return variable names as stored in __NAMES__ tag
;        - EXTRA -> return information stored in __EXTRA__ tag. This
;             information is always returned as a structure
;        - NVARS -> return number of variables, i.e. tags that do not begin
;             with '__'
;        - HOMOGENEOUS -> return tag indices of tags with identical number of
;             elements (only those can be combined to an array with Stru2Arr).
;             This keyword honors the RefIndex keyword.
;        - NUMERIC -> return tag indices of numeric structure tags
;        - TYPE -> return variable type of structure tags. For non-variable
;             tags (whose name begin with '__') a -1 is returned
;        
;        REFINDEX -> indicates the tag index to compare the number of elements
;             to (default is the first variable index).
;
; OUTPUTS:
...
; with subject "IDL routine struinfo"
;-------------------------------------------------------------


function StruInfo,stru,Names=Names,Original_Names=Original_Names,Extra=Extra, $
                  NVars=NVars,Homogeneous=Homogeneous,Numeric=Numeric,RefIndex=RefIndex, $
                  Type=Type
