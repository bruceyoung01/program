;-------------------------------------------------------------
; $Id: struaddvar.pro,v 1.1.1.1 2003/10/22 18:09:38 bmy Exp $
;+
; NAME:
;        STRUADDVAR (function)
;
...
;
; CALLING SEQUENCE:
;        newstru = StruAddVar (oldstru, newvar [,newname] )
;
; INPUTS:
;        OLDSTRU -> the exisiting structure. This must eb a structure, 
...
;             use Arr2Stru first.
;
;        NEWNAME -> The name of the new variable. Only used if NEWVAR is
;             no structure.
;
; KEYWORD PARAMETERS:
...
;
; OUTPUTS:
;        A structure that combines the information from OLDSTRU and NEWVAR.
;
; SUBROUTINES:
;
; REQUIREMENTS:
...
;
; NOTES:
;        NEWNAME (or the tag names from NEWVAR) will be added to the __NAMES__
;            tag if present. __EXTRA__ entries will be combined only if tags
;            within __EXTRA__ structures are different. If __EXTRA__ contains
;            a non-structure variable it will be converted to a structure with
;            tag name 'EXTRA_N' where N is a number from 1-9, A-Z (the first
;            tag is just 'EXTRA').
;
; EXAMPLE:
;        nstru = StruAddVar(stru, findgen(100), 'DUMMY' )
;
;        x = { a:0L, b:strarr(10), c:findgen(100), __EXTRA__:'test' }
...
; with subject "IDL routine struaddvar"
;-------------------------------------------------------------
