;-------------------------------------------------------------
; $Id: ncdf_read.pro,v 1.1.1.1 2003/10/22 18:09:36 bmy Exp $
;+
; NAME:
;        NCDF_READ
;
...
function Valid_TagName, arg
      ...
      t = strrepl(arg,'-','_')
      t = strrepl(t,'+','P')
      t = strrepl(t,'$','S') ; (bmy, 10/21/03: changed "D" to "S")
      t = strrepl(t,'*','A') ; (bmy, 10/21/03: changed "S" to "A")
      t = strrepl(t,'&','_')
      t = strrepl(t,' ','_')
      t = strrepl(t,'@','_')
      t = strrepl(t,':','_') ; (bmy, 5/22/02, added)
      t = strrepl(t,'=','_') ; (bmy, 5/22/02, added)
      t = strrepl(t,'#','_') ; (bmy, 5/22/02, added)
      t = strrepl(t,'(','_') ; (bmy, 10/21/03, added)
      t = strrepl(t,')','_') ; (bmy, 10/21/03, added)
...
function ncdf_varnames, ncid, VarDimId=vardimid
    ...
    for i=0L,nstru.nvars-1 do begin
       ...
       if keyword_set(verbose) $
          then print,'Variable '+vardesc.name+'  Dim-IDs: '  $
