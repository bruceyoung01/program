; $Id: click_map.pro,v 1.2 2004/03/26 17:02:24 bmy Exp $
;-------------------------------------------------------------
;+
; NAME:
;        click_map
...
;
; CALLING SEQUENCE:
;        click_map, <minfo | ginfo > [,i,j] [,keywords]
;
; INPUTS:
...
;
;        WINDOW -> window number to plot into. Normally a new window is opened
;            with the /free option. Use a specific window number to overlay the
;            map onto existing data plotted with tvimage (see example).
;
;        DATA -> a data array with dimensions matching your model grid
;            (no error checking on this!) If DATA contains data, the value
;            at the selected grid box, and a statistic for neighbouring grid boxes
;            will be displayed together with the corrdinates.
;
; OUTPUTS:
...
; EXAMPLES:
;        (1)
;        click_map,CTM_TYPE('GEOS1')
;
;        display world map and have fun. 
;
;        (2)
;        click_map,CTM_TYPE('GEOS1',resolution=2),I,J,LIMIT=[0.,-180.,90.,-30.]
;
;        Display map of North America and select grid boxes for GEOS 2x2.5 grid.
;        Indices of last point are returned in I and J.
;
;        (3)
;        click_map,CTM_TYPE('GISS_II_PRIME'),I,J,center=[42.,-72.], $
;          /non_interactive
;        print,I,J
;
;        returns grid box index for Harvard Forest in GISS CTM II' (21,33)
;        without displaying a map
;
;        (4)
;        click_map,CTM_TYPE('GISS_II'),I,J,edge=[-25.,170.,0.,-100.], $
;          WE_INDEX=we,SN_INDEX=sn,/non_interactive
;
;        returns [ 69, 70, 71, 0, 1, ... , 15 ] in WE and [ 15, 16, ..., 21 ]
;        in SN. I is [ 70, 16 ], and J is [ 16, 22 ]. Note that I, J refer
;        to CTM (= FORTRAN) indices, whereas WS and SN are IDL array indices.
;        
;        (5) overlay interactive map onto data displayed with tvimage
;        you should create a generic modelinfo structure in this case.
;        Note: replace xdim, ydim with the dimensions of your data array!
...
;        click_map,ginfo,i,j,window=0,data=data
;
...
; or bmy@io.harvard.edu with subject "IDL routine click_map"
;-------------------------------------------------------------


pro click_map,info,i,j,center=center,   $
         ...
         window=window_nr,data=data,shells=shells,idmap=idmap
...
@gamap_cmn
     ...
     if (n_elements(info) eq 0) then $
        info = select_model(default=default)
     ...
     if (keyword_set(non_interactive)) then begin
        ...
        if (keyword_set(get_coordinates)) then begin
            if (n_elements(i) eq 0 OR n_elements(j) eq 0) then begin 
                print,'click_map: ** Need to supply I and J index !'
                return
            endif
        ...
        ; ---------------------------------------------------------------
            ...
            if (n_elements(CENTER) lt 2 AND  $
                n_elements(EDGE) lt 4) then begin
                  print,'click_map: ** Must provide CENTER or '+ $
                        'EDGE coordinates!'
                  return
            endif
            ...
            if (n_elements(CENTER) ge 2) then begin
               ...
               ;-------------------------------------------------------------
               ; Prior to 1/24/01:
               ;if (ind(0) lt 0) then print,'**Y** (please report to mgs !)'
               ;-------------------------------------------------------------
               j = ((ind(n_elements(ind)-1)) > 0) < ginfo.jmx
               ind = where(xedge le center[1])
               ;-------------------------------------------------------------
               ; Prior to 1/24/01:
               ;if (ind(0) lt 0) then print,'**X** (please report to mgs !)' 
               ;-------------------------------------------------------------
...
; print,'#CENTER:',center
               ; get "true" coordinates via recursive call to click_map
               click_map,ginfo,i,j,/get_coordinates,CENTER=center,  $
...
; print,'#CENTER2:',center
            ...
            ; -----------------------------------------------------------
               ...
               J = intarr(2)
               CTR = EDGE[0:1]
               click_map,ginfo,newi,newj,CENTER=ctr,/non_interactive
               I[0] = newi
               J[0] = newj
...
; print,'##i0,i1 (click_map):',I[0],I[1]
     ...
     map_continents,color=1,/usa,/coasts
...
;; plot map
     ...
     if n_elements(idmap) eq 0 then idmap = replicate(0.,ginfo.imx, ginfo.jmx)


         for jy = 0, ginfo.jmx-1 do begin
         for ix = 0, ginfo.imx-1 do begin

           ; plot index (IDL array convention)
         
           if idmap[ix,jy] eq 0. then ccolor = 13 else  ccolor = 3

           ; plot rectangle with grid box edges
           px = xedge(ix)
           py = ginfo.yedge(jy) > (-89.8)
           dx = ginfo.di
           dy = ginfo.dj
           py2 = (py+dy) < 89.8
...
;               color=2
           polyfill,[px,px+dx,px+dx,px,px],[py,py,py2,py2,py], $
               color=ccolor
         endfor
         endfor
...
;        device,copy=[0,0,!d.x_size,!d.y_size,0,0,pixwin]
        ...
        if (!mouse.button eq 1 AND finite(mx1) AND $
           ...
           click_map,ginfo,i,j,center=[my1,mx1],/non_interactive
           ...
           if idmap[pi,pj] eq 0. then begin
              idmap[pi,pj] = 1.
              ccolor = 3
           end else begin
              idmap[pi,pj] = 0.
...
;               color=2
           ...
           click_map,ginfo,i,j,center=center,edge=edge,/get_coordinates, $
                 /non_interactive
