;$Id: ctm_extract.pro,v 1.1.1.1 2003/10/22 18:06:03 bmy Exp $
;-----------------------------------------------------------------------
;+
; NAME:
...
;
; KEYWORD PARAMETERS:
;        MODELINFO -> The model info structure returned from function 
;             CTM_TYPE.  
;
;        GRIDINFO -> The grid info structure returned from function
;             CTM_GRID.
;
...
;
;        PRANGE -> A two-element vector specifying the min and max 
;             pressure levels to be included in the extracted data block.
;
...
; EXAMPLE:
;        (1)
;        ModelInfo  = CTM_TYPE( 'GEOS1' )
;        GridInfo   = CTM_GRID( ModelInfo )
;        DataRegion = CTM_EXTRACT( DataCube, ModelInfo, DataInfo, $
;                                  Lon=[-180,0], Lat=[-30,30], $ 
;                                  Lev=[1,10] )
;
;               ; Extracts from a GEOS-1 4x5 data cube a region 
...
; or mgs@io.harvard.edu with subject "IDL routine ctm_extract"
;-----------------------------------------------------------------------
...
function CTM_Extract, Data, X, Y, Z,                          $
   ...
   if ( Index ) then begin
      ...
      ;
      ; NOTE: Uncommented this line (bmy, 7/30/01)
      ...
      ;------------------------------------------------------------------
      ; Prior to 7/30/01:
      ; Bug fix: now extract proper latitude range (bmy, 7/30/01)
      ;NewLat      = Default_Range( Lat,      [ -90.,90.],   /Limit2          )
      ;------------------------------------------------------------------
      NewLat      = Default_Range( Lat,      Def_Y_Range, /Limit2          )
