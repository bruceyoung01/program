; $Id: ctm_read3db_header.pro,v 1.2 2004/01/29 19:33:37 bmy Exp $
;-------------------------------------------------------------
;+
; NAME:
;        CTM_READ3DB_HEADER
...
;### skip = 4L * ( dim[0]*dim[1]*dim[2] ) + 8L    ( 8 is for EOL in fortran )
       ...
       if (count eq 0) then begin
          ...
          ;-------------------------------------------------------------------
          ; Prior to 11/21/03:
          ; Bug w/ GEOS3_30L in CTM_TYPE is fixed now (bmy, 11/21/03)
          ;;### Kludge for GEOS-3 reduced grid w/ 30 layers! (bmy, 7/10/03)
          ;if ( ModelName eq 'GEOS3_30L' ) then begin
          ;   ModelInfo.Name    = 'GEOS3_30L'
          ;   ModelInfo.NLayers = 30
          ;endif
          ;-------------------------------------------------------------------
