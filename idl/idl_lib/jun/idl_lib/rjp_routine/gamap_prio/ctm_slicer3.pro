...
;
; PURPOSE:
;       Widget based application to visualize 3D data for GAMAP.
...
;       bmy, 1/19/99 - ** RENAMED to CTM_SLICER3***
;                    - store a local copy of the IDL library
;                      routine SLICER3 for use with GAMAP 
;       mgs, 26 Jan 1999: - changed PICKFILE to DIALOG_PICKFILE
;
;
;-
...
; ### Change slice color table to EOS-B
;   if (N_ELEMENTS(cTab) le 3L) then cTab = [3,1,8]
   if (N_ELEMENTS(cTab) le 3L) then cTab = [27,27,27]
...
pro Viz3D_DrawCube, sMainState, SKIPCUBE=skipCube, SKIPAXIS=skipAxis, $
   ...
   if (((sMainState.cubeOn) and not(KEYWORD_SET(skipCube))) or $
      ...
      ;### Draw the map here (bmy, 1/14/99)
      ;### This causes XMANAGER to hang
      ;Map_Set, 0, 0, 0, /Grid, /Cont, /T3d, /NoErase
         ...
         if ((event.press ge 1) or $
            ...
            if (zFront gt (-32765)) then begin
               ...
               if (nZ gt zBack) then begin
                  ...
                  ; ### Coordinates in pixel space (bmy, 1/14/99)
...
ON_IOERROR, IO_BAD
   ...
   case uVal of
      ...
      'wLoadBttn': begin ; Load new data from a file.
         ; Get file name.
         dataFile = DIALOG_PICKFILE(GROUP=event.top, /READ, FILTER='*.dat', $
                             /MUST_EXIST)
         WIDGET_CONTROL, /HOURGLASS
...
;                    PTR_FREE, (*(sMainState.hDisplayList))[i]
      ...
      'wSaveTiffBttn': begin ; Save tiff image.
         tiffFile = DIALOG_PICKFILE(GROUP=event.top, /WRITE, FILTER='*.tif', $
                             FILE='slicer3.tif')
         WIDGET_CONTROL, /HOURGLASS
      ...
      'wSaveSubsetBttn': begin ; Save subset.
         dataFile = DIALOG_PICKFILE(GROUP=event.top, /WRITE, FILTER='*.dat', $
                             FILE='slicer3.dat')
         WIDGET_CONTROL, /HOURGLASS
...
; ### renamed to CTM_SLICER3 (bmy, 1/19/99)

pro CTM_Slicer3, hData3D, DATA_NAMES=dataNames, DETACH=detachView, $
   ...
   if (maxHand ge 0L) then begin
      ...
      for i=0, maxHand do begin
         ...
         if NOT(PTR_VALID(hData3D(i))) then begin
            PRINT, "CTM_Slicer3: data pointer is not valid."
            return
         endif

         neData = N_ELEMENTS((*(hData3D(i))))
         if (neData le 0L) then begin
            PRINT, "CTM_Slicer3: data pointer does not contain valid data."
            return
         endif
         ...
         if (szData(0) ne 3L) then begin
            PRINT, "CTM_Slicer3: data does not have 3 dimensions."
            return
         endif else begin
            if (min(szData(1:3)) le 1L) then begin
               PRINT, $
                  "CTM_Slicer3: all 3 dimensions of data must be greater than 1."
               return
            endif
         endelse
         ...
         if (i eq 0) then sz1 = szData else begin
            ...
            if (diffDim(0) ne (-1)) then begin
               PRINT, $
                  "CTM_Slicer3: all data sets must have the same dimensions."
               return
            endif
         endelse
   ...
   if (WIDGET_INFO(groupLead, /VALID_ID)) then begin
      wMainBase = WIDGET_BASE(TITLE='GAMAP 3D Data Visualizer (CTM_Slicer3)', $
