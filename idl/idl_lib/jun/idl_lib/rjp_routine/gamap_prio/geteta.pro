; $Id: geteta.pro,v 1.2 2004/01/29 19:33:40 bmy Exp $
;-----------------------------------------------------------------------
;+
; NAME:
;        GETETA
...
;
; EXAMPLE:
;        EETA = GETSIGMA( 'GEOS4' PSURF=984.0, /EDGES );
;             ; assigns GEOS-1 eta edges to array EETA
;
;        CETA = GETSIGMA( 'GEOS4', /CENTER )
...
; with subject "IDL routine geteta"
;-----------------------------------------------------------------------
...
function GetEta, MName, NLayers, PSurf=PSurf, Center=Center, Edges=Edges
   ...
   ; GISS-II-PRIME model
   ;====================================================================  
   else if ( StrPos( ModelName, 'GISS_II' ) ge 0 ) then begin
