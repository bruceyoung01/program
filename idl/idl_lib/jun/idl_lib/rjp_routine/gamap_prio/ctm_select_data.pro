; $Id: ctm_select_data.pro,v 1.2 2004/01/29 19:33:38 bmy Exp $
;-------------------------------------------------------------
;+
; NAME:
...
; with subject "IDL routine ctm_select_data"
;-------------------------------------------------------------
...
@gamap_cmn.pro
    ...
    ;------------------------------------------------------------------
    ; Prior to 11/19/03:
    ; Don't hardwire to 100 here (bmy, 11/19/03)
    ;if (n_elements(tracer) eq 0) $
    ;   then xtracer = indgen(99)+1 $
    ;   else xtracer = tracer
    ;------------------------------------------------------------------
    if ( N_Elements( Tracer ) eq 0 )           $
