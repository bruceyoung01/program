; $Id: gamap.pro,v 1.5 2004/06/03 17:58:10 bmy Exp $
;-------------------------------------------------------------
;+
; NAME:
;        GAMAP
...
; print,'### RANGE=',range,'  longitude=',longitude
   ...
   if (Ind[0] ge 0) then   $
      LowerEdge = Centers[Ind[n_elements(Ind)-1] < (NEdges-2)]  $
   else  $
      LowerEdge = Centers[0]     ; can't go beyond first edge

   Ind = where(Edges ge Range[1] )
   if (Ind[0] ge 0) then   $
      UpperEdge = Centers[(Ind[0]-1) < (NEdges-2)]  $
   else  $
      UpperEdge = Centers[NEdges-2]     ; can't go beyond last edge
...
function GAMAP_AutoYRange, $
              D, AvMode, TotalMode, Lon, Lat, Level
...
pro GAMAP_SelectPlotType, $
   ...
   case ( NDim ) of
      ...
      ; 0 Dimensions: Print value of data
      0: begin
      ...
      ; 1 Dimension: Create a line plot
      1: begin
         Message, 'GAMAP will create a line plot.', /Info, /NoName
         Ptype = -1
      end

      ; 2 Dimensions: query user for contour plot or pixel plot options
      2: begin
         print
      ...
      ; 3 Dimensions: Show 3-D Isopleth map (bmy, 1/22/01)
      3: begin
...
pro GAMAP, DiagN,                                                    $
           ...
           Do_TIFF=Do_TIFF,             TIFFFileName=TIFFFileName,   $
                                        ;---------------------------------
                                        ; Prior to 12/6/02:
           Frame0=Frame0,      $        ;  Multiple=Multiple,           $
                                        ;------------------------------------
           Do_MPEG=Do_MPEG,             MPEGFileName=MPEGFileName,   $
   ...
   ;=================================================================
   ; GAMAP: Main program
   ; Primitive user-interface for ctm_plot
   ...
   if (FirstTime) then begin
      print,'============================================================='
      print
      print,'   WELCOME  TO  G A M A P  -----  Version 2.02, Jun 2004'
      print
   ...
   ;-----------------------------------------------------------------------
   ; Prior to 11/13/03:
   ; Remove XINTERANIMATE option (bmy, 11/13/03)
   ;; Default setting for DO_XINTERANIMATE
   ;; (easy way to turn this off to save memory)
   ;Do_XInterAnimate = CreateAnimation
   ;-----------------------------------------------------------------------
   ...
   CTM_Get_Data, DataInfo, DiagN, File=FileName,  $
      Tracer=Tracer, Tau0=Tau0, Status=2, _EXTRA=e 
...
Do_The_Plot:
   ...
   for N = 0, N_Elements( D ) - 1 do begin
      ...
      ;==============================================================
      ; Call CTM_PLOT to make the plot for each data block
      ;==============================================================
      ...
      CTM_Plot, D[N].Category,                       $
         ...
         c_colors=c_colors,                          $
         sample=sample,       unit=ustr,             $
         YRange=YRange,       Polar=Polar,           $
...
;------------------------------------------------------------------------------
; Prior to 11/13/03:
; Comment out XINTERANIMATE options for now (bmy, 11/13/03)
;      ;=================================================================
;      ; Save animation frames into the IDL XINTERANIMATE routine
;      ;
;      ; NOTE: You may shut this option off in the "gamap.defaults" file
;      ; by setting DO_XINTERANIMATE = 0.  In practice, there usually 
;      ; isn't enough memory to animate more than about 10 frames at
;      ; once.  It may be better to save to GIF or PNG frames and then
;      ; concatenate those into a movie externally. (bmy, 12/9/02)
;      ;
;      ; NOTE: Not sure why this doesn't work on PC's (bmy, 12/9/02)
;      ;=================================================================
;      if ( Do_Animation AND Do_XInterAnimate )  then begin
;
;         ; Set up XINTERANIMATE when N=0 (bmy, 10/14/99)
;         if ( N eq 0 ) then begin
;            XInterAnimate, $
;               Set=[ !D.X_SIZE, !D.Y_SIZE, N_Elements( D ) ], /Track
;         endif
;
...
;
;            ; 16 or 24 bit color
;            ThisFrame = TvRd( True=3 )
;            ThisFrame = Color_Quan( ThisFrame, 3, R_Cur, G_Cur, B_Cur, $
;                                    Colors=!D.TABLE_SIZE, CUBE=3 ) 
;        
;            ; Make sure color 0 is WHITE and color 1 is BLACK
;            R_Cur[0]=255 & G_Cur[0]=255 & B_Cur[0]=255
;            R_Cur[1]=0   & G_Cur[1]=0   & B_Cur[1]=0
;
;         endif else begin
...
;         endelse
;
;         ; Add each frame to XINTERANIMATE (bmy, 10/14/99)
;         XInterAnimate, Frame=N, Image=ThisFrame
;      endif
;------------------------------------------------------------------------------
   ...
   ; Prior to 11/13/03:
   ;;====================================================================
   ;; To save on resources, undefine THISFRAME and DATA
   ;;====================================================================
   ;Undefine, ThisFrame
   ;Undefine, Data
   ;-------------------------------------------------------------------------
   ...
   ;-------------------------------------------------------------------------
   ; Prior to 11/13/03:
   ; Comment out XINTERANIMATE option for now (bmy, 11/13/03)
   ;;=================================================================
   ;; If we are doing on-screen animation, call XINTERANIMATE
   ;; to display the sequence of frames stored in the FRAME array
   ;;=================================================================
   ;if ( Do_Animation AND Do_XInterAnimate ) then begin
   ;   XInterAnimate, 10, _EXTRA=e
   ;endif
   ;-------------------------------------------------------------------------
