; $Id: bpch_link.pro,v 1.3 2004/06/03 17:58:07 bmy Exp $
;-----------------------------------------------------------------------
;+
; NAME:
;        BPCH_LINK
...
; with subject "IDL routine bpch_link"
;-----------------------------------------------------------------------
...
pro Bpch_Link, InFiles, OutFile, $
   ...
   ; External functions
   FORWARD_FUNCTION CTM_Grid, CTM_Type, MFindFile
   ...
   ;--------------------------------------------------------------------
   ; Prior to 5/28/04:
   ;; If INFILES contains more than one element, use that as the
   ;; file list.  Otherwise get of list of files via MFINDFILE.
   ;if ( N_Elements( InFiles ) eq 1 )     $
   ;   then InList = MFindFile( InFiles ) $
   ;   ;then InList = FindFile( InFiles ) $
   ;   else InList = InFiles
   ;--------------------------------------------------------------------
   ...
   for N = 0L, N_Elements( InList ) - 1L do begin
      ...
      Open_File, ThisFile, Ilun, /F77, /Get_LUN
