; $Id: gamap_init.pro,v 1.2 2004/01/29 19:33:40 bmy Exp $
;-------------------------------------------------------------
;+
; NAME:
;        GAMAP_INIT
;
; PURPOSE:
;        Initialize global common block for Global Atmospheric
;        Model output Analysis Package (GAMAP). This routine is 
;        automatically called when gamap_cmn.pro is included in a
;        file ( @gamap_cmn.pro ), but it executes only once.
;        User preferences are read from the file gamap.defaults
;        in the current directory or the directory where 
;        gamap_init.pro resides.
;
; CATEGORY:
;        CTM routines
;
...
;
; REQUIREMENTS:
;        This routine uses FILE_EXIST, EXTRACT_PATH, 
;        and OPEN_FILE
;
; NOTES:
...
; or bmy@io.harvard.edu with subject "IDL routine gamap_init"
;-------------------------------------------------------------
...
pro gamap_init,debug=newdebug
   ...
   ;---------------------------------------------------------------------------
   ; Prior to 11/13/03:
   ; This is now obsolete (bmy, 11/13/03)
   ;CreateAnimation     = '0'       ; Now toggles XINTERANIMATE (bmy, 12/10/02)
   ;---------------------------------------------------------------------------
