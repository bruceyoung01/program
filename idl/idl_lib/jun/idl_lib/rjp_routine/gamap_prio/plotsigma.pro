; $Id: plotsigma.pro,v 1.1.1.1 2003/10/22 18:06:03 bmy Exp $
;-------------------------------------------------------------
;+
; NAME:
;        PLOTSIGMA
...
;
; REQUIREMENTS:
;        References tools from GAMAP and TOOLS packages.
;         
; NOTES:
;        
; EXAMPLE:
;        PLOTSIGMA, /PLEFT
;            MODELNAME=['GISS_II', 'GEOS1', 'GEOS_STRAT', 'FSU' ], $
...
; with subject "IDL routine plotsigma"
;-------------------------------------------------------------
...
pro PlotSigma, ModelName,                    $
   ...
   if ( not keyword_set( ModelName   ) ) then ModelName = [ 'GEOS1' ]
   if ( not keyword_set( SurfP       ) ) then SurfP     = 1010.0
   if ( not keyword_set( YRange      ) ) then YRange    = [ 0, 31.5 ]
   ...
   Plot, XRange, YRange1, /NoData,                                   $
      ...
      YTick_Get=YTickG,       Color=CC.BLACK,  Title='Sigma Levels', $
      _EXTRA=e
   ...
   for N = 0, NMax - 1 do begin
      ...
      for L = 0, CC-1 do begin
         ...
         ;---------------------------------------------------------------------
         ;if ( L eq MInfo.NTROP AND ) then begin
         ;   oplot, X, [ Edge(L), Edge(L) ], Line=0, Thick=5, Color=BLACK
         ;endif else begin
         ;   oplot, X, [ Edge(L), Edge(L) ], Line=0, Thick=LThick, Color=LColor
         ;endelse
         ;---------------------------------------------------------------------
