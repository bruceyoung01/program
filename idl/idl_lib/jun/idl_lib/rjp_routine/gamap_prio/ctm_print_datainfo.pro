; $Id: ctm_print_datainfo.pro,v 1.1.1.1 2003/10/22 18:06:04 bmy Exp $
;-----------------------------------------------------------------------
;+
; NAME:
;        CTM_PRINT_DATAINFO
...
pro ctm_print_datainfo,DataInfo,fields=fields,  $
   ...
   for i=0,n_elements(DataInfo)-1 do begin
      ...
      if ( Ind[0] ge 0 ) then begin
         ...
         ;--------------------------------------------------------------
         ; Prior to 10/24/02:
         ;GEOS = ( Family eq 'GEOS' OR Family eq 'GENERIC' )
         ;--------------------------------------------------------------
      ...
      s = [ s , string(DataInfo[i].category,   $
             ...
             ;---------------------------------------------------------------
             ; Prior to 2/14/01:
             ; Decrease TRCNAME to A7, increase DATE to I10.10 (bmy, 2/13/01)
             ;format='(A8,1X,I4,1X,A10,I6,1X,A12,1X,F9.2,"(",I8.8,") ",A)') ]
             ;---------------------------------------------------------------
