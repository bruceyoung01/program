 ...
 for ic = 0, n_elements(tracerV)-1 do begin
  ...
  for ik = 0, 11L do begin
    ...
    data = ctm_burden('IJ-AVG-$',Filename=file, $
              ...
              if ik eq 0 then $
                   sum = total(Cmole*12.01,3)/area  $
              else sum = sum + total(Cmole*12.01,3)/area
  endfor
             Avgdata = sum / 12.
             Undefine, sum
...
;   MULTIPANEL,Position=p,margin=0.01
  ...
  MarginV = [0.15,0.1,0.15,0.1]
   ...
   Xyouts, 0.15, 0.92, 'OC  Avg 0.0022', /normal, color=1
   Xyouts, 0.15, 0.42, 'BC  Avg 0.0005', /normal, color=1
