...
;      rdtrpps.pro
;PURPOSE:
;      Read in trpps depth from GEOS-DAS  
;
;KEYWORD PARAMETERS
...
;
;- 
function rdtrpps,dd=dd,dsn=dsn,iskip=iskip,date=date,hd=hd,iread=iread,ilun=ilun,$
 f77=f77,fdshift=fdshift 

if n_elements(dd) eq 0 then dd = ''
if n_elements(dsn) eq 0 then dsn = 'stratf.b970701.e970707.trpps_xdr'
...
ilmm = 144 & ijmm = 91 
hd = fltarr(3) & trpps = fltarr(ilmm,ijmm) 
...
for i=0,iskip do readu,ilun,hd,trpps 
...
case fdshift of
0: 
else: trpps = shift(trpps,ilmm/2,0) 
endcase
