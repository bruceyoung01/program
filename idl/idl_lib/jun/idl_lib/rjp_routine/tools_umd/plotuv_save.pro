...
u11 = [u1,u1] & v11 = [v1,v1] 
i1 = where(loncen ge lons(0)) & i1 = i1(0) 
i2 = where(loncen ge lons(1)) & i2 = i2(0) & if (i2 eq -1) then i2 = ilmm-1 
j1 = where(latcen ge lats(0)) & j1 = j1(0) 
j2 = where(latcen ge lats(1)) & j2 = j2(0) 
dx = loncen(i2) - loncen(i1)
iii = 0 
...
!p.multi = [remframes,xstack,ystack]                  
;  Draw wind vectors 
if (ilmm-i2 eq 1) then x2 = loncen(0)+360. else x2 = loncen(i2) 

dy = latcen(j2)-latcen(j1) 
!p.thick=2.5 
velovect,uu,vv,loncen(i1:i2),latcen(j1:j2),length=length,/noerase,xstyle=1,$
 xrange=[loncen(i1),x2],yrange=[latcen(j1),latcen(j2)],xtickn=xtickn,ytickn=ytickn,$
...
aa = loncen(i1)+ findgen(xticks+1) * (x2-loncen(i1)) / xticks 
...
for i=0,yticks do xyouts,loncen(i1)-dx/40.,aa(i),slat(i),alignment=1.,charsize=1.5,charthick=2.5
xyouts,(loncen(i1)+loncen(i2))*0.5,latcen(j2)+dy/10.,title,alignment=0.5,charsize=1.5,charthick=2.5

title2 = 'Max wind = ' + strtrim(fix(zz+0.5),2) + ' m/s' 
xyouts,(loncen(i1)+loncen(i2))*0.5,latcen(j1)-dy/4.,title2,alignment=0.5,charsize=1.2,charthick=2.5 
