...
PRO cobar_ps1, BOTTOM=bottom, CHARSIZE=charsize, COLOR=color, DIVISIONS=divisions, $
...
IF KEYWORD_SET(vertical) THEN BEGIN
   IF N_ELEMENTS(location) EQ 0 THEN location = [0.88, 0.15, 0.95, 0.95]
ENDIF ELSE BEGIN
   IF N_ELEMENTS(location) EQ 0 THEN location = [0.15, 0.88, 0.95, 0.95]
ENDELSE 

; Scale the color bar.
   ;bbar = BYTSCL(bar, TOP=ncolors-1) + bottom
   bbar = bar(bottom:ncolors)

   ; for postscript file
   ; Display the color bar in the window.
   TV, bbar, location (0), location(1), xsize = location(2)-location(0), $
      ysize = location(3)-location(1), /NORMAL
