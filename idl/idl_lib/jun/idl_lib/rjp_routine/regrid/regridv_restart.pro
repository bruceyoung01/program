; $Id: regridv_restart.pro,v 1.4 2005/02/02 16:38:47 bmy Exp $
;-----------------------------------------------------------------------
;+
; NAME:
;        REGRIDV_RESTART
...
; with subject "IDL routine regridv_restart"
;-----------------------------------------------------------------------
...
pro RegridV_Restart, InFileName=FileName,     OutModelName=OutModelName, $
   ...
   for D = 0L, N_Elements( DataInfo ) - 1L do begin
      ...
      PsFileName = '~/IDL/regrid/PSURF/ps-ptop.' + CTM_NamExt( InType ) + $
                   '.'                           + CTM_ResExt( InType )
