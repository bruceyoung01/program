; $Id: ctm_make_datainfo.pro,v 1.2 2004/01/29 19:33:36 bmy Exp $
;------------------------------------------------------------------------
;+
; NAME:
...
;
; REQUIREMENTS:
;        Uses CREATE3DHSTRU, CREATE3DFSTRU, TAU2YYMMDD
;
; NOTES:
...
;        ; The user will be prompted for model type, diagnostics and 
;        ; tracer.
;   
;        ; Add a zonal mean difference data set (already in DATA) 
;        result = CTM_MAKE_DATAINFO(data,datainfo,fileinfo, $
;            model=ctm_type('GEOS1',psurf=1013.),diagn='ZONE-AVG', $
...
; or bmy@io.harvard.edu with subject "IDL routine ctm_make_datainfo"
;----------------------------------------------------------------------
...
function CTM_Make_DataInfo, Data, DataInfo, FileInfo,                $
   ...
   ;--------------------------------------------------------------------
   ; Prior to 11/19/03:
   ; DIAGSTRU.INDEX tagname does not exist anymore (bmy, 11/19/03)
   ;if ( stru.index lt 0 ) then stru.category = strtrim(diagn,2)
   ;--------------------------------------------------------------------
