; $Id: ctm_open_file.pro,v 1.7 2005/03/24 18:03:11 bmy Exp $
;-----------------------------------------------------------------------
;+
; NAME:
;        CTM_OPEN_FILE
...
function Get_Free_Lun, pFileInfo
   ...
   if ( Ptr_Valid( pFileInfo ) ) $
      then return, Max( (*(pFileInfo)).Ilun ) + 1  
...
Exit_Loop:
      ...
      ; netCDF file created by BPCH2NC
...
function Test_For_HDFEOS, FileName
   ...
   ;====================================================================
   ; Internal function TEST_FOR_HDFEOS tests if FILENAME is a HDF-EOS 
   ; file.  The testing algorithm is as follows:  (bmy, 11/25/03)
   ;
   ...
   if ( EOS_GD_InqGrid(  FileName, List ) gt 0 ) then return, 301
   ;if ( EOS_SW_InqSwath( FileName, List ) gt 0 ) then return, 302
   ;if ( EOS_SW_InqPoint( FileName, List ) gt 0 ) then return, 303
...
function Test_For_GMAO, Ilun, Debug=Debug
   ...
   ; field file.  The testing algorithm is as follows: (bmy, 11/18/03)
   ;
   ...
   ; (4) Returns 104 if FILENAME is a GMAO met field file; -1 otherwise.
   ;
   ; NOTES: 
...
Try_The_Read_Again:
   ...
   if ( StrMid( TmpStr, 0, 2 ) eq 'G4' ) then begin

      ;-----------------------------------------------------------------
      ; If we have found the GEOS-4 ident string, then we know that
      ; this is a GMAO met field file so set FILETYPE=104 (bmy, 12/12/03)
      ;-----------------------------------------------------------------
      FileType = 104
...
function Test_For_Binary, Ilun, Debug=Debug, Swap_Binary=Swap_Binary
   ...
   ; algorithm is as follows:  (bmy, 11/18/03, 8/24/04)
   ;
...
function File_Parse, Ilun, Ftype, ThisFileInfo, ThisDataInfo, _EXTRA=e
   ...
   ; given file, based on the value of FTYPE. (bmy, 11/25/03, 8/24/04)
   ;
   ...
   ;  104   GMAO binary met field file         CTM_READ_GMAO
   ...
   ;  301   HDF-EOS file w/ gridded data       CTM_READ_EOSGR
   ;  302   HDF-EOS file w/ satellite swath    not yet implemented
   ;  303   HDF-EOS file w/ point data         not yet implemented
   ;====================================================================
...
pro CTM_Open_File, FileName, ThisFileInfo, ThisDataInfo,  $
   ...
   ; Then test for HDF-EOS format (to be implemented later)
   if ( FileType eq 0 ) then FileType = Test_For_HDFEOS( FileName )

   ; Then test for HDF file (to be implemented later)
   ...
   if ( FileType le 0 ) then begin
      ...
      ; Open the file assuming big endian
      Open_File, FileMask, Ilun, $
         Default=DefaultPath, Filename=FileName, Swap_Endian=Little_Endian()
      ...
      ;---------------------------------
      ; Test for byte-swapped BPCH file
      ;---------------------------------
      if ( FileType eq 0 ) then begin
         FileType = Test_For_Binary( ilun, Debug=Debug, /Swap_Binary )
      endif
