; $Id: ctm_column_du.pro,v 1.1.1.1 2003/10/22 18:06:02 bmy Exp $
;-----------------------------------------------------------------------
;+
; NAME:
;        CTM_COLUMN_DU
;
; PURPOSE:
;        Calculates columns in Dobson Units for a given
;        CTM tracer.
;
; CATEGORY:
;        CTM_Tools
;
; CALLING SEQUENCE:
;        RESULT = CTM_COLUMN_DU( [ FileName ], [ DiagN ], [ Keywords ] )
;
; INPUTS:
...
function CTM_Column_DU, DiagN,                                        $
   ...
   Success = CTM_Get_DataBlock( Press, 'PRESS-AV',          $
                                Use_FileInfo=Use_FileInfo,  $
                                Use_DataInfo=Use_DataInfo,  $
   ...
   ; If not successful, try again w/ category name "PS-PTOP"
   if ( not Success ) then begin
   ...
   if ( GEOS and N_Elements( TropFileName ) gt 0 ) then begin
      ...
      for I = 0L, IMX - 1L do begin
         ...
         if ( L lt Trop[IREF, JREF] ) then begin
            ...
            ; Compute column density in the grid box [molec/cm2]
            C[I, J, L] = Data[I, J, L] * AD[I, J, L] / A_Cm2[I, J]      
         endif
      endfor
      endfor
      endfor
      ...
      for L = 0L, LMX - 1L do begin
         ...
         ; Compute air mass
         AD[*, *, L] = Press[*, *] * DSIG[L] * G0_100 * A_M2[*, *] * XNumolAir 

         ; Compute concentration of tracer
         C[*, *, L]  = Data[*, *, L] * AD[*, *, L] / A_Cm2[*, *]   
      endfor

   endelse

   ;===================================================================
   ; Construct columns in molec/cm2 -- sum O3 in the vertical
   ; Convert to Dobson Units (1 DU = 2.69e16 molec/cm2).
   ;===================================================================
