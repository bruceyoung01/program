 ...
 A =  [[TmaxOMAA/stddev(TmaxOMA)], [TminOMAA/stddev(TminOMA)],$
        [PrecOMAA/stddev(PrecOMA)], [TmaxLNKA/stddev(TmaxLNK)], $
        [TminLNKA/stddev(TminLNK)], [PrecLNKA/stddev(PrecLNK)] ]     
 cov = correlate(transpose(A))
 print, 'cov = ', cov
 ...
 ; note: in this case both eigenvaluea and eigenvectors
 ; is divided by the correposing squre_root of eigenvalue.  
...
plot, findgen(31)+1, TmaxOMA, xtitle = 'day in July 2009', $
      ...
      xstyle=1, ystyle=1
Oplot,  findgen(31)+1, stddev(TmaxOMA)*reconstruct(*,0)+mean(TmaxOMA), linestyle=3

plot, findgen(31)+1, PrecOMA, xtitle = 'day in July 2009', $
      ytitle = 'Precip. in Omaha', yrange=[0, 3], xrange=[0, 32], $
      xstyle=1, ystyle=1
Oplot,  findgen(31)+1, stddev(PrecOMA)*reconstruct(*,2)+mean(PrecOMA), linestyle=3

 ; save image
 img = tvrd()
 write_png, 'table111_correlation.png', img 
