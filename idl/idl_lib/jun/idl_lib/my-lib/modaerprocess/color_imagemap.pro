pro color_imagemap, image, lat, lon, newimage = newimage, range = range, $
...
;the above ismodified by Jun Wang for color display
newimage = replicate( -1, ns, nl )
newimage( p( 0, * ) * ( ns - 1 ), p( 1, * ) * ( nl - 1 ) ) = $
  image
  
   result = where(image eq 12, count)
	  print, "=12 count", count
	  
   result = where(newimage eq 12, count)
	  print, "new=12 count", count
...
loc = where( newimage eq -1, count )
if ( count ge 1 ) and ( missing ge 0B) then newimage( loc ) = missing
...
result = where(newimage eq 12, count)
print, 'count =12 =', count, 'offset=', xoffset, yoffset
