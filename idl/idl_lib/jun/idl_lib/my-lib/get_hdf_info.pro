...
print,  'reading # of datasets and file attibutes in file'
...
print,  'No. of datasets :', ndatasets
print,  'No. of attributes :', Nattributes
...
for j = 0, nattributes-1 do begin
  ...
  print,  'File Attibute # ', + strtrim(j,2), ': ', thisAttr
endfor
...
for j = 0, ndatasets -1 do begin
   ...
   print,  'SDS No. ' + strtrim(j,2) , ': ', thisSDSName, '  SdsType = ',     SdsType
   ...
   ; select SDS data
   if (not (nd eq 1 and DimSize eq 0 )) then begin
   ...
   for k = 0, numAttrOfSDS-1 do begin
     ...
     print,  '  Data Attribute: ', thisAttrName, '   ',  thisAttrData 
   endfor
   print , 1,  ' '
  endif
endfor
