@ScaleMODIS.pro
PRO overlay3band_img, win_x, win_y, rred, ggreen, bblue, lat, lon, $
    np, nl, filedir, filename, histequal=histequal, cloud=cloud, $
    range = range, reflectance=reflectance
...
if not Keyword_Set(range) then range = [0.01, 1.0]

if Keyword_Set(cloud) then begin
red = scalemodis(rred, range=range, cloud=cloud) 
green = scalemodis(ggreen,range=range, cloud=cloud) 
blue = scalemodis(bblue, range=range, cloud=cloud)
endif else begin
red = scalemodis(rred, range=range ) 
green = scalemodis(ggreen,range=range) 
blue = scalemodis(bblue, range=range)
endelse
endif else begin
  red = rred
  green = ggreen
...
histequal = Keyword_Set(histequal)
if histequal then begin
red = hist_equal(red)
...
;  tvimage, scalemodis(newred, newgreen, newblue)
