...
set_plot,'ps'
device,filename='MODIS_06_' + sensor + '_' + 'all' +'texas.cloud.ps',/portrait,xsize=7.5, ysize=9,$
...
;if (  DayNum(i) eq DayS+1 or DayNum(i) eq DayS ) then begin 
totnl = 0
totnnl = 0
print, 'file # is ', endinx(i) - startinx(i)  +1

;if ( Endinx(i) -  startinx(i) ge 9 ) then endinx(i) = startinx(i) + 8  
