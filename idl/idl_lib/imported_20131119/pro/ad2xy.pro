 ...
 if (ctype[0] EQ '') then begin   
  ...
  if spherical then begin
      IF astr2 THEN BEGIN
          ...
          a0 = [0d0,180d0]  & d0 = [0d0, 0d0] ; test points
          wcssph2xy, [a,a0], [d,d0], xsi, eta, CTYPE = ctype, PV1 = astr.pv1, $
...
; as this fix doesn't work in that case.
 ...
 IF testing THEN BEGIN
     ...
     test = WHERE(astr.projection EQ ['CYP','CAR','MER','CEA','HPX'])
     cylindrical = test NE -1
     IF cylindrical THEN BEGIN
         crval = astr.crval
