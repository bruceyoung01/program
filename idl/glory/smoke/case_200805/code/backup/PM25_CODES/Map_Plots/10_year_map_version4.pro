hour=0
  ...
  mean_pm_1year = fltarr(n_total_sites,24,nyear)
  

  for iyear = 1999, 2008 do begin

    filename = 'Simple_New_RD_501_88502_'+strtrim(iyear,1)+'-0.txt'
    readcol, filedir+filename, state_id, county_id, site_id ... all_hour,all_pm

    for ihour = 0, 23 do begin
      
     this_hour = ihour*100L

     ; 
     indx_this_hour = where(all_hour eq this_hour, count_this_hour)

   
     if (count_this_hour ge 1) then begin
      ; assign data for specific hour
      pm_hour = all_pm[indx_this_hour]
      state_hour = state_id[indx_this_hour]
      county_hour = ...
      site_hour = ...
     endif 

     for isite = 0, n_total_sites do begin
     
      ; note "state_id_info" read from separate file
      index_site = where(sate_hour eq state_id_info[isite] $
                     and county_hour eq ... $
                     and site_hour eq ..., count_site)
   
      if (count_site ge 1) then
        ; 
        mean_pm_1year[isite,ihour,iyear] = mean(pm_hour[index_site])
     
     endif else begin
       mean_pm_1year[isite,ihour,iyear] = -999
     endelse
  ...
  ; do multi-year average

  mean_pm_allyears = fltarr(n_total_stites, 24)
  
  for ihour = 0, 23 do begin
  for isite = 0, n_total_sites do begin
    
    idx_year = where(mean_pm_1year[isite,ihour,*] gt 0,  count_year) 

    if (count_year ge 1) then begin
      mean_pm_allyears[isite,ihour] = mean( mean_pm_1year[isite,ihour,idx_year] )
    endif else begin
      mean_pm_allyears[isite,ihour] = -999
    endelse
