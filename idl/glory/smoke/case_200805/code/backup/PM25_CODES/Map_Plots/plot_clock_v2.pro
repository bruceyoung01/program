...
; 
; 
   PRO plot_clock_v2, lat_station, lon_station, pm_value, pm_stddev, $
       pm_min, pm_max, pm_min_hour, pm_max_hour, title

;  open plot device

   SET_PLOT, 'PS'
   DEVICE,FILENAME= 'figures_each_site/clock_v2.ps', $
          XSIZE=8.5, YSIZE=10, $
...
;  ----------------------------------
   ...
   FOR i = 0, N_total_station -1 DO BEGIN
    ...
    PLOTS, CIRCLE(lon_station[i], lat_station[i], r_mean), Color=color_index, thick=2.5
    ...
    ARROW,  lon_station[i], lat_station[i], $
            ...
            hsize = -0.40, color=6, thick=2.5, /data, /solid
