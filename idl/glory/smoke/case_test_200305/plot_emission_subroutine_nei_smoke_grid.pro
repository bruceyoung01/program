...
; procedure for plotting flames emission data
 pro plot_emission_subroutine_nei_smoke_grid, lat1, lon1, flux, stime, date
...
;  region_limit = [min(lat1), min(lon1), max(lat1), max(lon1)]
  xl = region_limit(1)
  xr = region_limit(3)
  yb = region_limit(0)
  yt = region_limit(2)
 ...
 ; set legend
    ...
    for i = 0, n_levels-1  do begin
     ...
     xyouts, xa+i*dx, ya+dy+0.005, barticks(i),charthick=3,color=16,/normal,$
             align=1.0
     xyouts, xa +i*dx+0.01, ya+dy-0.005, ' ton/hr ', /normal,$
              color=16, charthick=3, charsize=1.4

     xyouts, 0.3, 0.84, 'NEI05 + SMOKE  ' + date, /normal,charsize=1.4,color=16,charthick=3
