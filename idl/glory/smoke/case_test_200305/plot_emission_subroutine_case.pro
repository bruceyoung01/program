...
; procedure for plotting flames emission data
 pro plot_emission_subroutine_case, lat1, lon1, flux, stime
...
;  region_limit = [0, -130, 50, -40]
 ...
 !p.thick=3
  ...
  plot, [xl, xr], [yb, yt], /nodata, xrange=[xl, xr], $
         ...
         ytitle='Latitude (deg) ', $
         position= [0.20, 0.31, 0.95, 0.77], color=15,$
...
; maximum of color
   ...
   for i = 0, count-1 do begin
    if ( lon1(result(i)) gt xl and lon1(result(i)) lt xr and $
      ...
      plots, lon1(result(i)), lat1(result(i)), symsize=0.82, $
 ...
 ; set legend
    ...
    barticks = string(barticks, format='(I2)')
    for i = 0, n_levels do begin
     y = [ya, ya, ya+dy, ya+dy, ya ]
     x = [xa+i*dx, xa+i*dx+dx, xa+i*dx+dx, xa+i*dx, xa+i*dx]
     polyfill,  x, y, color = colors(1+i)-21,/normal
     ;PRINT, 'WW : ', colors
     ;xyouts,  xa+i*dx-0.03, ya+dy+0.005, barticks(i), charthick=3,color=16,/normal,$
     ;        align=0.5
    endfor
    FOR j = 0, 3 DO BEGIN
     xyouts,  xa+4*j*dx+0.05, ya+dy+0.005, barticks(4*j+1), charthick=3,color=16,/normal,$
             align=0.5
    ENDFOR
     ;xyouts, xa+2*i*dx, ya+dy+0.005, barticks(i-5),charthick=3,color=16,/normal,$
     ;        align=0.5
     xyouts, xa +i*dx, ya+dy-0.005, ' Gg ', /normal,$
              color=16, charthick=3, charsize=1.4

     xyouts, 0.25, 0.84, 'SMOKE Emission   ' + '20030420_0521 23 hours V2', /normal,charsize=1.4,color=16,charthick=3
