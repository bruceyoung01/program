...
; read PM data over different stations.
  pro  read_pm, filelist, day, time, monchar, $
   ...
   nouse = ' '
   stationidchar = ' '
   datadir = '/s1/data/wangjun/s7/pm_10/PM_OBS/processed/'
   np = 25
   maxnl = 90
   maxsta = 200
   oneline = fltarr(np)
   ...
   ; read the data station by station
     ...
     ; open filelist
	  ...
	  ; get the filename corresponding to id and month
	    stationfile = datadir + monchar + '_'+stationidchar + '.dat.asc'
           ; print, 'station file is =', stationfile
    ...
    ; return values
      ;result = where(tmpavg gt 0  and tmpavg lt 999, count)
      result = where(tmpdata gt 0  and tmpdata lt 999, count)
      if ( count gt 0 ) then begin
        data = tmpdata(result)
        flat = tmplat(result)
        flon = tmplon(result)
...
plots, [-100, -100], [28, 35], color=0, thick=4, linestyle=2
plots, [-100, -97], [28, 28], color=0, thick=4, linestyle=2
plots, [-96, -96], [28.52, 33.9], color=0, thick=4, linestyle=2
...
red   = [0,    0, 255, 255,    255,  153, 76 , 0]
green = [0,  228, 255, 126,      0,    0,  0 , 0]
blue  = [0,    0,   0,   0,      0,   76, 38 , 255]
...
set_plot,'ps'
device,filename='dailyavg_airs_tx.ps',/portrait,xsize=6, ysize=8,$
xoffset=1,yoffset=1.5,/inches, /color, bits=8
Load_Color_Table

; read data, get right lat, lon, and time.
  filelist = './AIRSTX_SITEID.txt'
...
cal_aqicate, dailyavg, pmcolors

; based on the location, plot different symbols, not good idea
; just draw several lines
;for ik = 0, n_elements(flon) -1 do begin

; Norht west, filled diamond
;if ( flon(ik) lt -100 ) then $ 
;  plots, flon(ik), flat(ik), psym=sym(4, 1), color=pmcolors(ik), symsize=2.0

;souther
; if ( flat(ik) lt 28 ) then $ 
;  plots, flon(ik), flat(ik), psym=sym(1, 1), color=pmcolors(ik), symsize=1.8
;central, filled square 
; if ( flat(ik) gt 28 and flon(ik) gt -100 and flon(ik) lt -96 ) then $ 
;  plots, flon(ik), flat(ik), psym=sym(5, 1), color=pmcolors(ik), symsize=1.5
;east, traingle
; if ( flon(ik) gt -96 ) then $ 
