...
; procedure for plotting flames emission data
 pro plot_emission_subroutine_AOD_grid_monthly_plot_d, lat1, lon1, maxlat, minlat, maxlon, minlon, flux, date
  ...
  plot, [xl, xr], [yb, yt], /nodata, xrange=[xl, xr], $
   ...
   ; normalize flux data
     ...
     if ( count gt 0 ) then begin
        ...
        maxvalue = 0.06
        n_levels = 6
        flux = 20+fix((flux-minvalue)/((maxvalue - minvalue)/(n_levels)))*8
      endif
   PRINT, 'DD : ', flux
...
; maximum of color
 ...
 ; set legend
    ...
    barticks = string(barticks, format='(F8.1)')
    for i = 0, n_levels-1  do begin
     ...
     xyouts, 0.35, 0.84, 'MODIS AOD ' + date, /normal,charsize=1.4,color=16,charthick=3
