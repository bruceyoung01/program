...
; procedure for plotting flames emission data
 pro plot_emission_subroutine_LST_firenum, lat1, lon1, maxlat, minlat, maxlon, minlon, flux, date
  ...
  plot, [xl, xr], [yb, yt], /nodata, xrange=[xl, xr], $
   ...
   ; normalize flux data
     ...
     if ( count gt 0 ) then begin
        ...
        maxvalue = 6.
        n_levels = 6
        flux = 13+fix((flux-minvalue)/((maxvalue - minvalue)/(n_levels)))*8
      endif
   PRINT, 'DD : ', flux
...
; maximum of color
 ...
 ; set legend
    ...
    for i = 0, n_levels-1  do begin
     ...
     xyouts, 0.15, 0.84, 'MODIS fire number ' + date, /normal,charsize=1.4,color=16,charthick=3
