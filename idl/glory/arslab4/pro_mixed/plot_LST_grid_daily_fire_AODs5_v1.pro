...
;  CLOSE, lun
  ...
  FOR i = 0, n-1 DO BEGIN
    ...
    PRINT, 'MOD04 AOD file : ',afilename(i)
    ;PRINT, 'AOD : ', aaod
    ...
    as5  = FLTARR(np,nl)
    aodindex = WHERE(xaod lt 0.5 and xaod ge 0.0, acount)
    PRINT, '0.0<AOD<0.5 : ', acount
    IF (acount gt 0) THEN BEGIN
    as5(aodindex) = xaod(aodindex)
    ...
    FOR ni = 0, nfire-1 DO BEGIN
      IF (xaod(fire_sample(ni),fire_line(ni)) ge 0.0 and xaod(fire_sample(ni),fire_line(ni)) lt 0.5) THEN BEGIN
        ...
        FOR k = 0, gridsize_lat-1 DO BEGIN
          FOR l =0, gridsize_lon-1 DO BEGIN
            ...
            index01 = where(xlat ge grid_lat(k)-0.25 $
                        ...
                        and as5  gt 0.0, acount )

           IF (acount gt 0) THEN BEGIN
           meanas5(l,k) = mean(as5[index01])
           ENDIF ELSE BEGIN
           meanas5(l,k) =  0
           ENDELSE
           PRINTF, lun2, grid_lat(k), grid_lon(l), meanas5(l,k), acount
         ENDFOR
       ENDFOR
  FREE_LUN, lun2
