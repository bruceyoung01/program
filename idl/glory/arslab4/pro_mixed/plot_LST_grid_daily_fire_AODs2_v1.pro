...
;  CLOSE, lun
  ...
  FOR i = 0, n-1 DO BEGIN
    ...
    as2  = FLTARR(np,nl)
    aodindex = WHERE(xaod lt 0.2 and xaod ge 0.0, acount)
    PRINT, '0.0<AOD<0.2 : ', acount
    IF (acount gt 0) THEN BEGIN
    as2(aodindex) = xaod(aodindex)
    ...
    FOR ni = 0, nfire-1 DO BEGIN
      IF (xaod(fire_sample(ni),fire_line(ni)) ge 0.0 and xaod(fire_sample(ni),fire_line(ni)) lt 0.2) THEN BEGIN
        ...
        FOR k = 0, gridsize_lat-1 DO BEGIN
          FOR l =0, gridsize_lon-1 DO BEGIN
            ...
            index01 = where(xlat ge grid_lat(k)-0.25 $
                        ...
                        and as2  gt 0.0, acount )

           IF (acount gt 0) THEN BEGIN
           meanas2(l,k) = mean(as2[index01])
           ENDIF ELSE BEGIN
           meanas2(l,k) =  0
           ENDELSE
           PRINTF, lun2, grid_lat(k), grid_lon(l), meanas2(l,k), acount
         ENDFOR
       ENDFOR
  FREE_LUN, lun2
