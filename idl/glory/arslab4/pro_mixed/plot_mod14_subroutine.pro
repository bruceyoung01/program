...
; procedure for plotting flames emission data
 pro plot_mod14_subroutine, lat1, lon1, flux
 ...
 !p.multi=[0, 1, 2]
;  region_limit = [min(lat1)-2, min(lon1)-2, max(lat1)+2, max(lon1)+2]
  region_limit = [15, -95, 25, -85]
;  region_limit = [10, -115, 45, -65]
;  region_limit = [-40, 135, -20, 155]
  xl = region_limit(1)
  xr = region_limit(3)
  yb = region_limit(0)
  yt = region_limit(2)
 ...
 ; set legend
    ...
    for i = 0, n_levels-1  do begin
     ...
     xyouts, xa+i*dx, ya+dy+0.005, barticks(i),charthick=3,color=16,/normal,$
             align=1.0
     xyouts, xa +i*dx+0.01, ya+dy-0.005, ' ', /normal,$
              color=16, charthick=3, charsize=1.4

     xyouts, 0.30, 0.84, '     MODIS LAND SURFACE TEMPERATURE', /normal,charsize=1.4,color=16,charthick=3
