...
;  CLOSE, lun
  ...
  lun = 95
  lun1= 96
  OPENW, lun, filedirres + 'lst_' + date(0)+ '_22', /get_lun
  filename = Allfilename(startinx(j):endinx(j))

  OPENW, lun1, filedirres + 'fire_' + date(0)+ '_22', /get_lun
  ...
  FREE_LUN, lun
        ...
        FOR k = 0, gridsize_lat-1 DO BEGIN
          FOR l =0, gridsize_lon-1 DO BEGIN
            tmplst = 0.0
            ccount = 0
            index01 = where(firelat ge grid_lat(k)-0.1 $
                        and firelat le grid_lat(k)+0.1 $
                        and firelon ge grid_lon(l)-0.1 $
                        and firelon le grid_lon(l)+0.1 $
