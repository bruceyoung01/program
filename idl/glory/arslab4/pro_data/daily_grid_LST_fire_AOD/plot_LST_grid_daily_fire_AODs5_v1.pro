...
;  CLOSE, lun
  ...
  meanans5lst = FLTARR(gridsize_lon,gridsize_lat)
  meanfs5lst  = FLTARR(gridsize_lon,gridsize_lat)
  meanas5lst   = FLTARR(gridsize_lon,gridsize_lat)
  meanas5     = FLTARR(gridsize_lon,gridsize_lat)
  meanaods5   = FLTARR(gridsize_lon,gridsize_lat)
  ...
  OPENW, lun, filedirres + 'ans5lst_' + date(0), /get_lun
  filename = Allfilename(startinx(j):endinx(j))

  OPENW, lun1, filedirres + 'afs5lst_' + date(0), /get_lun
  ffilename= fAllfilename(fstartinx(j):fendinx(j))

  OPENW, lun2, filedirres + 'aods5_' + date(0), /get_lun
  ...
  alon    = FLTARR(anp,anl)
  aaod    = FLTARR(anp,anl)
  ans5lat = FLTARR(np, nl*(n+1))
  ans5lon = FLTARR(np, nl*(n+1))
  ans5lst = FLTARR(np, nl*(n+1))
  ...
  FOR i = 0, n-1 DO BEGIN
    ...
    aods5  = FLTARR(np,nl)
    as5  = FLTARR(np,nl)
    aodindex = WHERE( xaod gt 0.0 and xaod lt 0.5, acount)
    PRINT, '0.0<AOD<0.5 : ', acount
    IF (acount gt 0) THEN BEGIN
    aods5(aodindex) = xaod(aodindex)
    ...
    FOR ni = 0, nfire-1 DO BEGIN
      IF (xaod(fire_sample(ni),fire_line(ni)) gt 0.0 and xaod(fire_sample(ni),fire_line(ni)) lt 0.5) THEN BEGIN
    ...
    ENDFOR
      ...
      ans5lat(0:(np-1), (nl*i):(nl*(i+1)-1)) = tmpanlat
      ans5lon(0:(np-1), (nl*i):(nl*(i+1)-1)) = tmpanlon
      ans5lst(0:(np-1), (nl*i):(nl*(i+1)-1)) = tmpanlst
  ...
  ENDFOR
        ...
        FOR k = 0, gridsize_lat-1 DO BEGIN
          FOR l =0, gridsize_lon-1 DO BEGIN
           ...
           meanas5lst(l,k) =  0
           ENDELSE
           PRINTF, lun3, grid_lat(k), grid_lon(l), meanas5lst(l,k), acount
         ENDFOR
       ENDFOR
  ...
  FREE_LUN, lun3
        ...
        FOR k = 0, gridsize_lat-1 DO BEGIN
          FOR l =0, gridsize_lon-1 DO BEGIN
            ncount = 0
            index01 = where(ans5lat ge grid_lat(k)-0.25 $
                        and ans5lat le grid_lat(k)+0.25 $
                        and ans5lon ge grid_lon(l)-0.25 $
                        and ans5lon le grid_lon(l)+0.25 $
                        and ans5lst  gt 0.0, ncount )

           IF (ncount gt 0) THEN BEGIN
           meanans5lst(l,k) = mean(ans5lst[index01])
           ENDIF ELSE BEGIN
           meanans5lst(l,k) =  0
           ENDELSE
           PRINTF, lun, grid_lat(k), grid_lon(l), meanans5lst(l,k), ncount
         ENDFOR
       ENDFOR
  ...
  FREE_LUN, lun
        ...
        FOR k = 0, gridsize_lat-1 DO BEGIN
          FOR l =0, gridsize_lon-1 DO BEGIN
           ...
           meanfs5lst(l,k) = mean(firelst[index01])
           ENDIF ELSE BEGIN
           meanfs5lst(l,k) =  0
           ENDELSE
           PRINTF, lun1, grid_lat(k), grid_lon(l), meanfs5lst(l,k), fcount
         ENDFOR
       ENDFOR
        ...
        FOR k = 0, gridsize_lat-1 DO BEGIN
          FOR l =0, gridsize_lon-1 DO BEGIN
            ...
            index01 = where(xlat ge grid_lat(k)-0.25 $
                        ...
                        and aods5  gt 0.0, aodcount )

           IF (acount gt 0) THEN BEGIN
           meanaods5(l,k) = mean(aods5[index01])
           ENDIF ELSE BEGIN
           meanaods5(l,k) =  0
           ENDELSE
           PRINTF, lun2, grid_lat(k), grid_lon(l), meanaods5(l,k), aodcount
         ENDFOR
       ENDFOR
  FREE_LUN, lun2
