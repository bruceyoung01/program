...
; procedure for plotting flames emission data
 pro plot_emission_subroutine_LST_fire_no_monthly, lat1, lon1, maxlat, minlat, maxlon, minlon, flux, date
...
;  region_limit = [0, -130, 50, -40]
 ...
 !p.thick=3
  ...
  plot, [xl, xr], [yb, yt], /nodata, xrange=[xl, xr], $
   ...
   ;PRINT, 'AA : ', flux
   ; normalize flux data
     result = where(flux gt 0 and flux lt 270, count)
     if ( count gt 0 ) then begin
        ...
        maxvalue = 0.06
        n_levels = 6
        flux = 21+fix(((flux-minvalue)/((maxvalue - minvalue)/(n_levels)))*8)*8
      endif
   PRINT, 'DD : ', flux
; set the value of flux to maximum value of color, 
; if the value of flux is larger than the maximum of color
   for i = 0L, count-1 DO BEGIN
    IF ( flux(i) le 68 ) THEN BEGIN
        flux(i) = flux(i)
    ENDIF ELSE BEGIN
        flux(i) = 68
    ENDELSE
 ...
 ; set legend
    ...
    barticks = string(barticks, format='(F8.2)')
    ...
    for i = 0, n_levels-1  do begin
     ...
     xyouts, 0.20, 0.84, 'MODIS LST(fire)-LST(no)  ' + date, /normal,charsize=1.4,color=16,charthick=3
