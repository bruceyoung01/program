...
PRO truecolor_img_marker, red=red, green=green, blue=blue, $
 ...
 ; redraw the map with noerase opition
   map_set, /noerase, latdel = latdel , londel = londel,  $
         /grid, charsize=1.0, mlinethick = 2,$
         limit = region, color = 12, /label, $
;         latlab = latlab, lonlab = lonlab, $
;         /label, latalign=0, lonalign = 1, $
        position = [0.05, 0.05, 0.98, 0.98]

   map_continents, /hires, /coasts, /usa, color=12
...
;              endelse 
 ...
 FOR I = 0, N_ELEMENTS(LLAT)-1 DO BEGIN
  PLOTS, LLAT(I), LLON(I), PSYM = SYM(13), SYMSIZE = 5, COLOR = 200
 ENDFOR
