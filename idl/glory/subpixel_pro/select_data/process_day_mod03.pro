...
PRO process_day_mod03, infile, Nday, AllFileName, StartInx, EndInx, $
  ...
  nday_max = 366
   ...
   for i = 0, Nfile-1 do begin
      ...
      ; from file name to julian day
         ...
         JulianD = fix(strmid (OneF, 11, 3))
;         print, 'oneF = ', oneF  
         YearC = fix(strmid (OneF, 7, 4))

         NxF = AllFileName(i+1)                           ; next day
         NxJD = fix ( strmid ( NxF, 11, 3))
       ...
       ; judget if on the same day
          if ( JulianD ne NxJD  ) then begin
             ...
             MON(DI) = fix(Month)
             Date(DI) = fix(Day) 
             TimeS(DI) = strmid(AllFileName(SSINX), 15, 4) 
             TimeE(DI) = strmid(AllFileName(I), 15, 4)
             DayNum(DI) = tmpday
             DAYNAME(DI) = Month + Day + $
                           'JD' + strmid (AllFileName(SSINX), 10, 4)
             SSINX = i+1
             DI = DI + 1
          endif
    endfor
