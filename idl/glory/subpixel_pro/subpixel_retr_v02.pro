...
;------------------------------------------------------------------------------
; SAVE THE OUTPUT AS NC FILE
; DEFINE ATTRIBUTE FOR EACH VARIABLE
  modis_fire_area_attr = {units:'m^2'}
  rarea_modis_attr     = {units:'m^2'}
  final_tfire_attr     = {units:'K'}
  true_frp_attr        = {units:'MW'}
  modis_frp_attr       = {units:'MW'}
  frp_flux_attr        = {units:'Wm^-2'}
  modis_backt4         = {units:'K'}
  modis_backt11        = {units:'K'}
  
  ncfile   = outdir + 'subpixel_'+reg_date+'.nc'
  ncfields = 'jd; '       $
           + 'reg_date; ' $
           + 'nfp; '      $
           + 'fflat[nfp]; '                  $
           + 'fflon[nfp]; '                  $
           + 'modis_fire_corners[nfp,4,2]; ' $
           + 'modis_fire_area[nfp]; '        $
           + 'final_area_fract[nfp]; '       $
           + 'rarea_modis[nfp]; '  $
           + 'final_tfire[nfp]; '  $
           + 'true_frp[nfp]; '     $
           + 'modis_frp[nfp]; '    $
