...
if n_elements(linefills) eq 0 then lf=0 else lf=1
...
case lf of
0: plots,xp,yp,color=colors[i] $
      ,/normal,linestyle=linestyle[i],psym=symcat(psym[i]),symsize=symsize[i], $
      thick=thick[i]
1: begin
polyx=[(xp-(xspacing/2.)),(xp-(xspacing/2.)),(xp+(xspacing/2.)),(xp+(xspacing/2.))]; trying to get polyfill... ah I hate myself
polyy=[(yp-(yspacing/2.)),(yp+(yspacing/2.)),(yp+(yspacing/2.)),(yp-(yspacing/2.))]; trying to get polyfill... ah I hate myself
polyfill,polyx,polyy,/normal,color=colors[i],$
	/line_fill,thick=4,orientation=linefills[i]
plots,polyx[[0,1,2,3,0]],polyy[[0,1,2,3,0]],/normal,color=colors[i],thick=4
end
endcase
endif
endelse
; anything else I can end?
