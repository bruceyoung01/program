...
; select region limit
  cut_off = 0
  region_limit = [min(flat)-cut_off, min(flon)-cut_off, max(flat)+cut_off, max(flon)+cut_off]
...
; redraw the map with noerase opition
  map_set, /noerase, /continent, $
         charsize=0.8, mlinethick = 1,$
	limit = region_limit, color = black, /USA, $
       position =[xa, ya, xb, ybb], /CYLINDRICAL

   xyouts, win_x/2.,  win_y-18, '!6Visible   ' + strmid(filename, 0, 22), color=black, $
