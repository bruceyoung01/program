...
@/home/bruce/idl/IDLLIB/gamap2/gamap_util/gamap_util/tvmap.pro

;  DEFINE DATA DIRECTORY, RESULT DIRECTORY AND FILE NAMES LIST FOR MODIS
;  AOD DATA (TERRA OR AQUA)
   filedir    = '/home/bruce/zzperson/imran/2011_MOD04_HK/'
...
;                  ;TEXAS    ; WA     ; SAHEL   GLOBAL
   maxlat = 22.6   ;40.0     ; 45.0   ; 35.0     90.0
   minlat = 22.1   ;25.0     ; 30.0   ;-15.0    -90.0
   maxlon = 114.5  ;-90.0    ;-110.0  ; 45.0    180.0
   minlon = 113.8  ;-115.0   ;-130.0  ;-25.0   -180.0 
...
;  PLOT THE DAILY AOD GRID ONTO MAP
    ...
    TVMAP, transpose(aaod(j, *, *)), /grid,$
           ...
           title = 'AOD(Terra) 2011 '+STRMID(dayname(j),0,4),        $
           /cbar,DIVISIONS = 6, maxdata = maxaod, mindata = minaod,  $
           CBMIN = minaod, CBMAX = maxaod, /COAST, $
           MIN_VALID = minaod, /hires, lats = grid_lat, lons = grid_lon
    DEVICE, /close
    FREE_LUN, lun
   ENDFOR
...
;   SET_PLOT, 'ps'
;   DEVICE, filename =filedirres + 'plot_2011' + STRMID(dayname(0),0,2) + '.ps', $
;           xsize = 7., ysize=10, xoffset = 0.5, yoffset = 0.5, $
;           /inches, /color, bits=8
;   TVMAP, transpose(maod), /grid,$
;          LIMIT=[minlat, minlon, maxlat, maxlon], /ISOTROPIC, sample = 4, $
;          title = 'AOD(Terra) 2011 '+STRMID(dayname(0),0,2),        $
;          /cbar,DIVISIONS = 6, maxdata = maxaod, mindata = minaod,  $
