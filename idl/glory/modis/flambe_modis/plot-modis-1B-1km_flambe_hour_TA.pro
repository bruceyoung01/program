; $ID: plot-modis-1B-1km_flambe_hour_TA.pro V01 03/23/2012 16:26 BRUCE EXP$
;
;******************************************************************************
;  PROGRAM plot-modis-1B-1km_flambe_hour_TA PLOTS MODIS TRUE COLOR IMAGE 
;  OVERLAID WITH FIRE LOCATION FROM FLAMBE FIRE EMISSION IN EACH HOUR 
;  FROM BOTH TERRA AND AQUA.
;
;  VARIABLES:
;  ============================================================================
;  (1 )
...
@/home/bruce/program/idl/modis/code/flambe_modis/read_flambe_hour_TA.pro
...
; READ LATITUDE, LONGITUDE OF FIRE DOTS FROM FLAMBE EMISSION
  ...
  FOR J = 1, 24 DO BEGIN
   firelat = FLTARR(NMAX)
   firelon = FLTARR(NMAX)
   READ_FLAMBE_HOUR_TA, FIREFILEDIR, FIREFILELIST,            $
...
;   endelse
   ...
   truecolor_img, red=red, green=green, blue=blue,   $
              ...
              '_' + STRTRIM(STRING(J), 2), $
              firelat = firelat, firelon = firelon,$
              title=sate+string(year(i), format='(I4)') + Dayname(i) + $
              '_' + TimeS(i) + '_' + TimeE(i) + ' ' + STRING(J, FORMAT='(I2)') 
  ENDFOR
 ENDFOR
 END
