; $ID: plot-modis-1B-1km_flambe_hour_TA_dn.pro V01 03/23/2012 16:26 BRUCE EXP$
;
;******************************************************************************
;  PROGRAM plot-modis-1B-1km_flambe_hour_TA_dn PLOTS MODIS TRUE COLOR IMAGE 
;  OVERLAID WITH FIRE LOCATION FROM FLAMBE FIRE EMISSION IN EACH HOUR 
;  FROM BOTH TERRA AND AQUA WITH DAYTIME AND NIGHTTIME, SEPARATELY.
;
;  VARIABLES:
;  ============================================================================
;  (1 )
...
;  (2 ) MODIFIED FROM plot-modis-1B-1km_flambe.pro. (03/23/2012)
;  (3 ) MODIFIED FROM plot-modis-1B-1km_flambe_hour.pro. (03/23/2012)
;  (4 ) MODIFIED FROM plot-modis-1B-1km_flambe_hour_TA.pro. (03/27/2012)
;******************************************************************************


;  CALL SUBROUTINES
@/home/bruce/idl/IDLLIB/my-lib/mod1B/truecolor_img_TA_dn.pro
@/home/bruce/program/idl/modis/code/flambe_modis/read_flambe_hour_TA_dn.pro
...
; READ LATITUDE, LONGITUDE OF FIRE DOTS FROM FLAMBE EMISSION
  NMAX    = 50000
  FOR J = 1, 24 DO BEGIN
   dtfirelat = FLTARR(NMAX)
   dtfirelon = FLTARR(NMAX)
   ntfirelat = FLTARR(NMAX)
   ntfirelon = FLTARR(NMAX)
   dafirelat = FLTARR(NMAX)
   dafirelon = FLTARR(NMAX)
   nafirelat = FLTARR(NMAX)
   nafirelon = FLTARR(NMAX)
   READ_FLAMBE_HOUR_TA_dn, FIREFILEDIR, FIREFILELIST,                $
                        YEAR = IYEAR, MONTH = IMONTH, DAY = IDAY,    $
                        HOUR = J, SATELLITE1 = SATELLITE1,           $
...
;   endelse
   ...
   truecolor_img_TA_dn, red=red, green=green, blue=blue,   $
              ...
              '_hour_TA_dn' + STRTRIM(STRING(J), 2), $
              DTFIRELAT = DTFIRELAT, DTFIRELON = DTFIRELON,$
              NTFIRELAT = NTFIRELAT, NTFIRELON = NTFIRELON,$
              DAFIRELAT = DAFIRELAT, DAFIRELON = DAFIRELON,$
              NAFIRELAT = NAFIRELAT, NAFIRELON = NAFIRELON,$
              title=sate+string(year(i), format='(I4)') + Dayname(i) + $
              '_' + TimeS(i) + '_' + TimeE(i) + '_hour_TA_dn ' + STRING(J, FORMAT='(I2)') 
  ENDFOR
 ENDFOR
 END
