...
@../sub/process_day.pro
  ...
  fileres = '/mnt/sdb/data/CALIPSO/2008/CALIPSO/data/result/CAL_LID_L1-ValStage1-V3-01/1/'
...
; processing values
     ...
     ntbks = 2+(ttbks - minvalue)/(maxvalue - minvalue) * N_levels*10
...
; plot the profile
  ...
  MYCT, 33, ncolors =  14, range = [0.0, 1]

  CONTOUR, ntbks, alat, alt, nlevels=N_levels+2,  $
        xrange=[xl, xr], yrange=[yb, yt],  /fill, $
        levels=levels,$
        c_colors=FIX((ntbks-min(ntbks))/(max(ntbks)-min(ntbks))*255),xstyle=1, ystyle=1, color=1, $
        ytitle='!6Altitude (km) ', $
  ...
  ENDFOR
  COLORBAR, Position=[0.1275, 0.1075, 0.8535, 0.1425], Bottom=0, NColors=14, $
           Divisions=13, Minor=0, YTicklen=1, Range=[0,1], $
           /Right, Format='(I3)'
