...
@/home/bruce/program/idl/idl_lib/procedure/sahel/sub_read_mod04.pro
@/home/bruce/program/idl/idl_lib/procedure/sahel/plot_emission_subroutine.pro
@/home/bruce/program/idl/idl_lib/procedure/sahel/sub_LST_grid.pro
@/home/bruce/program/idl/idl_lib/procedure/sahel/process_day_aod.pro

;  DEFINE DATA DIRECTORY, RESULT DIRECTORY AND FILE NAMES LIST FOR MODIS
;  AOD DATA (TERRA OR AQUA)
   filedir    = '/home/bruce/sshfs/shw/data/satellite/MODIS/sahel/myd04/'
   filedirres = './'
...
;  MEAN OF AOD INTO ONE 1-DIMENSIONAL ARRAY
    ...
    dbaod = FLTARR(np, nl*(n+1))
    taod  = FLTARR(np, nl*(n+1))
    FOR i = 0L, n-1 DO BEGIN
     sub_read_mod04, filedir, filename(i), rlat, rlon, raod, rdbaod, np, nl
     index02 = WHERE(raod lt 0.0)
;print, raod(index02)
     raod(index02)   = 0.0
;print, raod(index02)
     index03 = WHERE(rdbaod lt 0.0)
     rdbaod(index03) = 0.0
...
;  FOR AOD
     ...
     dbaod(0:(np-1), (nl*i):(nl*(i+1)-1)) = rdbaod
    ENDFOR
    zaod = [aod, dbaod]
    taod = aod + dbaod;TOTAL(zaod, 1, /NAN)
    ...
    FOR k = 0, gridsize_lat-1 DO BEGIN
     FOR l =0, gridsize_lon-1 DO BEGIN
      ...
      index01 = where(lat ge grid_lat(k)-new_grid/2.0 $
                  ...
                  and taod gt 0.0, ccount )
      IF (ccount gt 0) THEN BEGIN
       meanaod(l,k)   = mean(taod[index01])
      ENDIF ELSE BEGIN
       meanaod(l,k) =  0
      ENDELSE
