...
;                           calculate and plot their difference.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  FOR i = 0L, nn_lines-1  DO BEGIN
   FOR j = 0L, nf_lines-1  DO BEGIN
    IF(STRMID(nfile(0, i), 5, 5) eq STRMID(ffile(1, j), 5, 5) and $
       STRMID(nfile(1, i), 5, 5) eq STRMID(ffile(0, j), 5, 5) and $
       nfile(3, i) eq jday) THEN BEGIN
       PRINT, i, j, jday, nfile(3, i)
       PRINT, STRMID(nfile(0, i), 5, 5), STRMID(ffile(1, j), 5, 5)
       PRINT, STRMID(nfile(1, i), 5, 5), STRMID(ffile(0, j), 5, 5)
       PRINTF, lun1, nfile(1, i), nfile(0, i), nfile(52, i), nfile(54, i), nfile(56, i), nfile(58, i)
       PRINTF, lun2, nfile(1, i), nfile(0, i), ffile(5, j), ffile(6, j)
...
; read the file of the differences bwtween the NOAA fire emission and FLAMBE fire emission
  ...
  fire_flambetarea = FLTARR(nfa_lines)
  ...
  FOR i = 0, nfa_lines-1 DO BEGIN
   ...
   fire_flambetarea(i) = TOTAL(fire_area(index1))
   PRINTF, lunc, lat_flambe(i), lon_flambe(i), fire_flambetarea(i), fire_flambeemi(i)
  ENDFOR
  FREE_LUN, lunc

  nna_lines = FILE_LINES(filedir + siyear + simonth + 'noaa')
  ...
  OPENR, lunb, filedir + siyear + simonth + 'noaa', /get_lun
  ...
  fire_noaa56emi(i) = tmpfire_noaa56*tmpfire_noaatarea
  fire_noaa58(i)= tmpfire_noaa58
  ENDFOR
  FREE_LUN, lunb

  area_index = WHERE(fire_noaatarea lt 1.0, area_count)
  fire_nflambetarea = fire_flambetarea(area_index)
  fire_fnoaatarea = fire_noaatarea(area_index)
;  re_flambe_noaa = REGRESS(fire_flambeemi, fire_noaa56emi, SIGMA = sigma, CONST = const, CORRELATION = correlation)
  re_flambe_noaa = REGRESS(fire_nflambetarea, fire_fnoaatarea, SIGMA = sigma, CONST = const, CORRELATION = correlation)
...
; plot the scatter plot of the NOAA fire emission and FLAMBE fire emission
  ...
  SET_PLOT, 'ps'
  DEVICE, filename =filedir + 'plot_' + siyear + simonth + '_flambe_noaa_scatter_area.ps', /portrait, xsize = 5.5, ysize=6, $
          xoffset = 0.5, yoffset = 1, /inches, /color, bits=8
  MyCt, /Verbose, /WhGrYlRd
  PLOT, fire_nflambetarea, fire_fnoaatarea, psym = sym(1), symsize = 0.5, color = 1;, $
